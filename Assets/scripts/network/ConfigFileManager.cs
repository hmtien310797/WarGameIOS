...
public class ConfigFileManager
{
		...
		if (m_fTime >= 0) 
		{
			...
			else if (m_fTime >= CHECK_WAIT_TIME)
			{
				...
				else if (m_wRequest != null)
				{
					...
					{
						SaveConfDataByWWW(m_wRequest.text);
						FinishCheckConf(true);
						m_fTime = -1;
					}
				}
	...
	void LoadConfData(string _filename)
	{
		...
		if (param != "") 
		{
			...
			{
				Dictionary<string, object> server = OurMiniJSON.Json.Deserialize((string)data["loginserver"]) as Dictionary<string, object>;
				foreach(string _key in server.Keys)
				{
					Dictionary<string, object> serverData = OurMiniJSON.Json.Deserialize((string)server[_key]) as Dictionary<string, object>;
	...
	public bool ParseConfData(bool _forceLocal = false)
	{
		...
		else
		{
			if (m_wRequest != null)
			{
				if (m_wRequest.bytes.Length <= 0)
				{
					DebugUtils.Log("Config File is invalid!");
				}
				else
				{
					bRet = parseConfData(m_wRequest.text);
					if (bRet)
					{
						SaveConfData(fileName);
					}
