class "MobaBMSelectHero"
{
}

function MobaBMSelectHero:__init__(trf,moveType, sortingConfig,fill,EnableignoreHeroAtts,climb_hero)
    self.transform = trf
    ...
    self.heroData = MobaSelectHero_PVP.GetSelectHeroData((EnableignoreHeroAtts~=nil and EnableignoreHeroAtts)and true or false)    
...
function MobaBMSelectHero:LoadSelectList(recommend)
    local heroIndex = 1
    local tmpherolist = {}
    local sheroData = MobaSelectHero_PVP.GetSelectHeroData((self.EnableignoreHeroAtts~=nil and self.EnableignoreHeroAtts)and true or false).memHero
    ...
    for _, v in ipairs(sheroData) do
        ...
        local heroMsg = MobaHeroListData.GetGeneralByUID(v) -- HeroListData.GetHeroDataByUid(v)
...
--[[
function MobaBMSelectHero:GetVaildHeroUid(uid)
...
function  MobaBMSelectHero:LoadHeroList()
...
--]]
function  MobaBMSelectHero:LoadUI(dontrefrushfight,needAuto)
    print("needAuto", needAuto,self.fill)
    if needAuto then
        if self.fill then
            MobaSelectHero_PVP.GetSelectHeroData((self.EnableignoreHeroAtts~=nil and self.EnableignoreHeroAtts)and true or false):UnselectAllHero()
            MobaSelectHero_PVP.AutoSelect(self.moveType,MapData_pb.SEntryData(),nil,(self.EnableignoreHeroAtts~=nil and self.EnableignoreHeroAtts)and true or false)
        else
            MobaSelectHero_PVP.GetSelectHeroData((self.EnableignoreHeroAtts~=nil and self.EnableignoreHeroAtts)and true or false):UnselectAllHero()
            MobaSelectHero_PVP.AutoSelect(self.moveType,TileInfo.GetTileMsg(),nil,(self.EnableignoreHeroAtts~=nil and self.EnableignoreHeroAtts)and true or false)
        end
    end
    --self.heroData:NormalizeData()
    MobaBattleMove.RecordTime("MobaBMSelectHero  self.heroData:NormalizeData()==============================")

    --self:LoadMyList()
    MobaBattleMove.RecordTime("MobaBMSelectHero self:LoadMyList()==============================")
    --[[
    ...
    MobaBattleMove.RecordTime("MobaBMSelectHero self:LoadSelectList()==============================")
    --AttributeBonus.CollectBonusInfo("SelectArmy")
    MobaBattleMove.RecordTime("MobaBMSelectHero AttributeBonus.CollectBonusInfo(SelectArmy)==============================")
    if dontrefrushfight ==  nil then
        print("[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[     MobaBMSelectHero:LoadUI")
        MobaBattleMove.RefrushTotalSoliderNum()
        MobaBattleMove.RecordTime("MobaBMSelectHero  MobaBattleMove.RefrushFight==============================")
    end
end
--[[
function UpdateHeroList(go, wrapIndex, realIndex)
    local self = MobaBattleMove.GetSelectHero()
...
function  MobaBMSelectHero:LoadMyList()
    ...
    MobaBattleMove.RecordTime("fill new newHeroListData==============================") 
...
function MobaBMSelectHero:SetIgnoreHero(ignoreHeros)
    self.ignoreHeros = {}
...
function MobaBMSelectHero:Awake(un_auto_hero)
    ...
    MobaBattleMove.RecordTime("MobaBMSelectHero UI 1==============================")
    for i = 1, 5 do
        self.selectedList[i] = {}
        local hero = {}
        ...
        --[[
        self.SetClickCallback(hero.btn.gameObject, function(go)
            MobaSelectHero_PVP.Show(self.moveType)            
    ...
    self.SetClickCallback(self.transform:Find("bg_title").gameObject,function() 
        if self.fill ~= nil and self.fill then
            MobaSelectHero_PVP.Show(self.moveType,MapData_pb.SEntryData(),
            (self.EnableignoreHeroAtts ~= nil and self.EnableignoreHeroAtts) and self.IgnoreHeroAtts or nil,function()
                local sh = MobaBattleMove.GetSelectHero()
                if sh ~= nil then
                    sh:LoadUI()
                end

                MobaBattleMove.CheckNeedSaveArchive()
             end)
        else
            MobaSelectHero_PVP.Show(self.moveType,TileInfo.GetTileMsg() or MapData_pb.SEntryData(),
            (self.EnableignoreHeroAtts ~= nil and self.EnableignoreHeroAtts) and self.IgnoreHeroAtts or nil,function()
                local sh = MobaBattleMove.GetSelectHero()
                if sh ~= nil then
                    sh:LoadUI()
    ...
    MobaBattleMove.RecordTime("init SelectedList ==============================")
    --self.heroPrefab = self.ResourceLibrary.GetUIPrefab("WorldMap/listitem_herocard_small0.6")
    MobaBattleMove.RecordTime("Load UI Prefab WorldMap/listitem_herocard_small0.6 ==============================")
    
    self:LoadUI(true,not un_auto_hero)
    MobaBattleMove.RecordTime("MobaBMSelectHero LoadUI==============================")
end
--[[
function MobaBMSelectHero:Start()
    self.heroListWrapContent.onInitializeItem = UpdateHeroList
    self.heroListWrapContent:ResetToStart()
end

function MobaBMSelectHero:Close()
