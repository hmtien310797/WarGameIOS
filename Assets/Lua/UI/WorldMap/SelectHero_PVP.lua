module("SelectHero_PVP", package.seeall)
...
local DefaultHeroAtts = {1004,100000021,1008}
local DefaultHeroAttsOutSea = {1102 ,1004,100000021,1008}
...
function GetSelectHeroData(ingoreHeroState)
    ...
    if heros ~= nil then
        for _, v in ipairs(heros) do
            ...
            if GeneralData.IsOutForExpediation(v) then
...
local function GetVaildHeroUid(uid)
    local baseHerodata = GeneralData.GetGeneralByUID(uid) -- HeroListData.GetHeroDataByUid(uid)
...
function AutoSelect(moveType, tileMsg, maxNumHeroSelected, ingoreHeroState)
    if not MissionListData.HasCompletedChapterMission(AUTO_SELECT_UNLOCK_MISSION_ID) then
		return
	end

    local selectHeroData = GetSelectHeroData(true)

    local suggestionRule = _ui and _ui.suggestionRule or TableMgr:GetExpediationSuggestionRule(moveType, tileMsg) -- TableMgr:GetExpediationSuggestionRule(HeroListData.GetHeroSuggestionRule(moveType, tileMsg))
    
    if suggestionRule == nil then -- 不需要推荐出征将军时先选取上次出征的将军后填充空位
        selectHeroData:UnselectAllHero()

        local lastExpediationHeroes = BattleMoveData.GetPreHeroList()

        for i = 1, #(lastExpediationHeroes) do
            local uid = GetVaildHeroUid(lastExpediationHeroes[i])
            if (_ui == nil or _ui.ignoreHeroAtts == nil) and (ingoreHeroState == nil or not ingoreHeroState) then
                if uid ~= nil and GeneralData.IsAvailableForExpediation(uid) then -- HeroListData.IsHeroAvailableForExpediation(uid) then
        ...
        for _, hero in ipairs(_ui and _ui.heroListData or GeneralData.GetSortedGenerals(suggestionRule and suggestionRule.sortingConfig)) do  -- HeroListData.GetSortedHeroes(suggestionRule and suggestionRule.sortingConfig)
...
function IsOutForExpediation(uid)
    if ignoreHeros == nil or ignoreHeros[uid] == nil then
        return GeneralData.IsOutForExpediation(uid)
    else
        return ignoreHeros[uid]~= nil or GeneralData.IsOutForExpediation(uid)
    end
end
...
local function LoadSelectList()
    ...
    for i, v in ipairs(_ui.heroListMsg) do
        if selectHeroData:IsHeroSelectedByUid(v.uid) then
            ...
            totalPower = totalPower + attributes[100] -- HeroListData.GetPowerByAttrList(attrList)
            ...
            if not Global.IsOutSea() then
                ...
                troopsValue = attributes[DefaultHeroAttsOutSea[1]] 
				setoutAttrList[1][2] = setoutAttrList[1][2] + troopsValue
    ...
    for attributeID, value in pairs(--[[allAttrList]]totalAttributes) do
        if not IngoreAtts(attributeID) then
	...
	local defultAttrs = Global.IsOutSea() and DefaultHeroAttsOutSea or DefaultHeroAtts
    for i = 1, (Global.IsOutSea() and 4 or 3) do
        if not IngoreAtts(defultAttrs[i]) then
        ...
        if i == 1 then
            if not Global.IsOutSea() then
                valueLabel.text = "+" .. (attrValue-attrValue%0.01) .. "%"
            else
...
local function LoadHeroList()
    ...
    for i, v in ipairs(_ui.heroListMsg) do
        ...
        local troopsValue = attributes[1102] or 0 -- HeroListData.GetTroops(heroMsg, heroData)
        troopsLabel.text = "+" .. troopsValue
        troopsLabel.color = troopsValue > 0 and Color.green or Color.white
		heroTransform:Find("quality1").gameObject:SetActive(Global.IsOutSea())
        ...
        moveLabel.text = "+" .. (moveValue-moveValue%0.01) .. "%"
        ...
        if GeneralData.IsDefending(heroMsg.uid) then
...
function LoadUI()
    _ui.suggestionRule = TableMgr:GetExpediationSuggestionRule(_ui.moveType, _ui.tileMsg)-- HeroListData.GetHeroSuggestionRule(_ui.moveType, _ui.tileMsg))
    _ui.heroListMsg = GeneralData.GetSortedGenerals(_ui.suggestionRule and _ui.suggestionRule.sortingConfig) -- HeroListData.GetSortedHeroes(_ui.suggestionRule and _ui.suggestionRule.sortingConfig)
