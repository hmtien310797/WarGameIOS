module("GuildMobaChat", package.seeall)
...
local function UpdateTabRedPoint()
	local new = GuildMobaChatData.GetUnreadPrivateCount()
	...
	local guildNew = GuildMobaChatData.GetUnreadGuildCount(0)
	bg_tab.guildRed.gameObject:SetActive(guildNew > 0 and curChanel ~= ChatMsg_pb.chanel_GuildMobaTeam )
	bg_tab.guildRed:Find("num"):GetComponent("UILabel").text = guildNew
	if curChanel == ChatMsg_pb.chanel_GuildMobaTeam then
		GuildMobaChatData.SetUnreadGuildCount(0)
	end
end
...
function GoMap(posx , posy)
	...
	GuildWarMain.LookAt(posx+ offx , posy+offy)
	GuildWarMain.SelectTile(posx+ offx , posy+offy)
...
function ReqTranslateGetUserContent(reqContent , reqLang, reqCount , resultcallback)
	...
	if reqCount > 3 then
		...
		coroutine.start(function()
			...
			reqCount = reqCount + 1
			local req = GuildMobaMsg_pb.GuildMobaUserTranslateTextRequest()
			req.clientLang = reqLang
			req.text:append(reqContent)
			Global.Request(Category_pb.GuildMoba, GuildMobaMsg_pb.GuildMobaTypeId.GuildMobaUserTranslateTextRequest, req, GuildMobaMsg_pb.GuildMobaUserTranslateTextResponse, function(msg)
...
local function SetChatInfoSystem(chatinfo, chatContent , item)
	...
	SetClickCallback(item.gameObject , function()
		...
		if str[1] == "jump" then
			...
			GUIMgr:CloseMenu("GuildMobaChat")
		end
...
local function SetChatInfoBattleReport(chatinfo, chatContent , item)
	...
	local teamData = TableMgr:GetMobaTeamData(chatinfo.sender.guildid)
	local roleData = TableMgr:GetMobaRoleTablebyID(chatinfo.sender.officialId)
	local guildLabel = "[f1cf63]【".. TextMgr:GetText(teamData.Name) .. "】[-]"
	local roleLabel = roleData and TextMgr:GetText(roleData.Name) or ""
	chatName.text = roleLabel .. guildLabel ..  "[ffffff]" ..  chatinfo.sender.name .. "[-]"
	...
	SetClickCallback(item.transform:Find(System.String.Format("{0}/btn_replay" , chatContent)).gameObject ,function(go)
		...
		reportData.baseid = baseid
		if fort > 0 then
		    reportData.subtype = Mail.MailReportType.MailReport_fort
		end
		Mail_share.Show(reportData, true)
	end)
...
local function SetContentAutoTranslate(chatinfo,chatContent, item)
	...
	if chatinfo.infotext ~= chatinfo.transtext and GuildMobaChatData.GetTranslateEnable() then
...
local function SetChatInfoNormalContent(chatinfo, chatContent , item)
	if GuildMobaChatData.AutoTranslate() and GuildMobaChatData.GetTranslateEnable() then
		...
		translateCoroutine = coroutine.start(function()
			while true do
				...
				if translateMapCount > 0 and not translateWaiting then
					...
					local req = GuildMobaMsg_pb.GuildMobaGetTranslateTextInfoRequest()
					...
					Global.Request(Category_pb.GuildMoba, GuildMobaMsg_pb.GuildMobaTypeId.GuildMobaGetTranslateTextInfoRequest, req, GuildMobaMsg_pb.GuildMobaGetTranslateTextInfoResponse, function(msg)
						if translateMap ~= nil then
...
function SetChatInfo(chatinfo , grid)
	...
	if chatinfo.gm then
		...
		local teamData = TableMgr:GetMobaTeamData(chatinfo.sender.guildid)
		local roleData = TableMgr:GetMobaRoleTablebyID(chatinfo.sender.officialId)
		local guildLabel = teamData and "[f1cf63]【".. TextMgr:GetText(teamData.Name) .. "】[-]" or ""
		local roleLabel = roleData and TextMgr:GetText(roleData.Name) or ""
		name.text = roleLabel .. guildLabel ..  "[ffffff]" ..  chatinfo.sender.name .. "[-]"
	end
	...
	SetClickCallback(touxiangBtn.gameObject , function()
		if chatContent == "left" then
			...
			MobaPersonalInfo.Show(chatinfo.sender.charid)
		end
	end)
...
function SendConditionContent(content , callback)
	local req = GuildMobaMsg_pb.GuildMobaChatInfoConditionSendRequest()
	req.type = content.type
	if req.type == ChatMsg_pb.ChatInfoConditionType_MobaGather then
	...
	Global.Request(Category_pb.GuildMoba, GuildMobaMsg_pb.GuildMobaTypeId.GuildMobaChatInfoConditionSendRequest, req, GuildMobaMsg_pb.GuildMobaChatInfoConditionSendResponse, function(msg)
...
function SendContent(content , callback)
	local req = GuildMobaMsg_pb.GuildMobaChatInfoSendRequest()
	req.info.chanel = tonumber(content.curChanel)
	req.info.infotext = content.content
	req.info.clientlangcode = content.languageCode or GUIMgr:GetSystemLanguage()--Global.GTextMgr:GetCurrentLanguageID()
	req.info.type = content.chatType
	req.info.senderguildname = content.senderguildname or ""
	print(content.curChanel , curChanel , content.groupid)
	req.info.spectext = content.spectext or ""
	if curChanel == ChatMsg_pb.chanel_MobaPrivate then
	...
	Global.Request(Category_pb.GuildMoba, GuildMobaMsg_pb.GuildMobaTypeId.GuildMobaChatInfoSendRequest, req, GuildMobaMsg_pb.GuildMobaChatInfoSendResponse, function(msg)
...
function NotifyChanelChat()
	if GUIMgr:FindMenu("GuildMobaChat") == nil then
		return
	end
	
	local chanelNewChat = GuildMobaChatData.GetChanelNewChat(curChanel , 10 , recvList)
	...
	if chanelNewChat ~= nil and #chanelNewChat > 0 then
		...
		local setCorount = coroutine.start(function()
			for _, v in pairs(chanelNewChat) do 
				if GuildMobaChatData.IsInBlackList(v.sender.charid) == false then
...
local function DeleteRecord(priName , priCharId)
	MessageBox.Show(TextMgr:GetText("Chat_priui1") , 
		function() 
			GuildMobaChatData.DeleteChatData(priName , priCharId)
...
local function CreateGroupMember(initlist , defaultMem)
	GroupSelectList.Show(initlist , function(addlist)
		...
		local req = GuildMobaMsg_pb.MsgChatDiscGroupCreateRequest()
		...
		Global.Request(Category_pb.GuildMoba, GuildMobaMsg_pb.GuildMobaTypeId.MsgChatDiscGroupCreateRequest, req, GuildMobaMsg_pb.MsgChatDiscGroupCreateResponse, function(msg)
...
UpdatePrivatePlaterContent = function (recName , recCharId , guildBanner , officialId, guildOfficialId)
	...
	UpdateChatContent(ChatMsg_pb.chanel_MobaPrivate)
end
...
local function UpdatePrivateNGroupListData()
	...
	Global.SetGuildMobaChatEnterChanel(ChatMsg_pb.chanel_GuildMobaWorld)
	...
	local contentItems = {}
	local sortTable = GuildMobaChatData.GetPrivateNew()
	for i=1 , #(sortTable) , 1 do
		local k = sortTable[i].name
		local v = GuildMobaChatData.GetPrivateChat(k)
		noitem = false
		--print("msg:" , k ,#(v))
		...
		local new = GuildMobaChatData.GetPlayerUnreadPrivateContent(priName , priCharId)
		...
		SetClickCallback(listitem:Find("bg").gameObject , function()
			--print("player rec:" , k , lastchat.sender.charid ,priCharId ,MainData.GetCharId())
	...
	for i=1 , #sortedCfg , 1 do
		...
		if contentItems[v.name] == nil then
			...
			local new = GuildMobaChatData.GetPlayerUnreadPrivateContent(k , v.charid)
...
function UpdateChatContent(updateChanel)
--	print(curChanel , updateChanel)
	if updateChanel ~= ChatMsg_pb.chanel_GuildMobaWorld and updateChanel ~= ChatMsg_pb.chanel_GuildMobaTeam and 
		updateChanel ~= ChatMsg_pb.chanel_MobaPrivate and updateChanel ~= ChatMsg_pb.chanel_discussiongroup then
	...
	--GuildMobaChatData.SaveChatList()
	chatContentList.privatePanel.gameObject:SetActive(false)
	bg_privateList.trf.gameObject:SetActive(false)
	chatContentList.scrollView.gameObject:SetActive(true)
	bg_bottom.bg.gameObject:SetActive(true)
	...
	if updateChanel ~= ChatMsg_pb.chanel_MobaPrivate and updateChanel ~= ChatMsg_pb.chanel_discussiongroup then
		privateShare = nil
	end

	local optChanel = updateChanel
	if updateChanel == ChatMsg_pb.chanel_discussiongroup then
		optChanel = ChatMsg_pb.chanel_MobaPrivate
	end
	Global.SetGuildMobaChatEnterChanel(optChanel)--记录聊天的页签
	...
	GuildMobaChatData.InitRecordIndex(curChanel, recvList.name , recvList.charid , recvList.groupid)
	...
	if updateChanel == ChatMsg_pb.chanel_MobaPrivate then
		--print("privateName:" , recvList.name , recvList.charid)
		chanelChat = GuildMobaChatData.GetPrivateRecordData(recvList.name , recvList.charid, 10 , false)
		chatContentList.privatePanel.gameObject:SetActive(true)
		GuildMobaChatData.SavePlayerChat(recvList.name , recvList.charid , true)
	elseif updateChanel == ChatMsg_pb.chanel_discussiongroup then
		chatContentList.privatePanel.gameObject:SetActive(true)
		chanelChat = GuildMobaChatData.GetChanelRecordChat(curChanel , 10 , nil , nil , recvList.groupid)
		local gd = GroupChatData.GetGroupData(recvList.groupid)
		local curChatid = (#chanelChat > 0) and chanelChat[#chanelChat].id or 0
		GroupChatData.SetGroupCurrentChatId(gd.id , curChatid)
	else
		chanelChat = GuildMobaChatData.GetChanelRecordChat(curChanel , 10 )
	end
	
	UpdateTabRedPoint()
	
	if updateChanel == ChatMsg_pb.chanel_GuildMobaTeam then
		GuildMobaChatData.SetUnreadGuildCount(0)
	end
	...
	if chanelChat == nil or #chanelChat <= 0 then
		...
		local setCorount = coroutine.start(function()
			...
			for _, v in pairs(chanelChat) do
				--print(v.infotext , v.sender.name , v.id)
				local chatitem = SetChatInfo(v)
			end
			coroutine.step()
			--cal height 
...
function UpdateRecordChatInfo()
	...
	local chanelChat = nil
	if curChanel == ChatMsg_pb.chanel_MobaPrivate then
		chanelChat = GuildMobaChatData.GetPrivateRecordData(recvList.name , recvList.charid, 10 , true)
	elseif curChanel == ChatMsg_pb.chanel_discussiongroup then
		chanelChat = GuildMobaChatData.GetChanelRecordChat(curChanel , 10 , nil , nil , recvList.groupid)
	else
		chanelChat = GuildMobaChatData.GetChanelRecordChat(curChanel , 10)
...
local function CheckLeaveUnion()
    if not UnionInfoData.HasUnion() then
        if not bg_tab[ChatMsg_pb.chanel_GuildMobaWorld].transform:GetComponent("UIToggle").value then
            UpdateChatContent(ChatMsg_pb.chanel_GuildMobaWorld)
            bg_tab[ChatMsg_pb.chanel_GuildMobaTeam].transform:GetComponent("UIToggle").value = false
        end
    end
end
...
function Awake()
	...
	_ui = {}
	bg_tab = {}
	bg_tab[ChatMsg_pb.chanel_GuildMobaWorld] = transform:Find("Container/bg_frane/bg_tab/btn_tabtype_5"):GetComponent("UIButton")
	transform:Find("Container/bg_frane/bg_tab/btn_tabtype_5/text1"):GetComponent("UILabel").text = TextMgr:GetText("ui_moba_72")
	transform:Find("Container/bg_frane/bg_tab/btn_tabtype_5/Animation/btn_lv1_down/tab_title_down1"):GetComponent("UILabel").text = TextMgr:GetText("ui_moba_72")
	SetClickCallback(bg_tab[ChatMsg_pb.chanel_GuildMobaWorld].gameObject, function(go)
		--curChanel = ChatMsg_pb.chanel_GuildMobaWorld
		--UpdateChatContent()
		UpdateChatContent(ChatMsg_pb.chanel_GuildMobaWorld)
	end)
	
	bg_tab[ChatMsg_pb.chanel_GuildMobaTeam] = transform:Find("Container/bg_frane/bg_tab/btn_tabtype_4"):GetComponent("UIButton")
	transform:Find("Container/bg_frane/bg_tab/btn_tabtype_4/text2"):GetComponent("UILabel").text = TextMgr:GetText("ui_moba_71")
	transform:Find("Container/bg_frane/bg_tab/btn_tabtype_4/Animation/btn_lv2_down/tab_title_down2"):GetComponent("UILabel").text = TextMgr:GetText("ui_moba_71")
	SetClickCallback(bg_tab[ChatMsg_pb.chanel_GuildMobaTeam].gameObject, function(go)
		--curChanel = ChatMsg_pb.chanel_GuildMobaTeam				
		--[[if not UnionInfoData.HasUnion() then
		...
		UpdateChatContent(ChatMsg_pb.chanel_GuildMobaTeam)
	end)
	...
	bg_tab[ChatMsg_pb.chanel_MobaPrivate] = transform:Find("Container/bg_frane/bg_tab/btn_tabtype_7"):GetComponent("UIButton")
	bg_tab[ChatMsg_pb.chanel_MobaPrivate].gameObject:SetActive(false)
	transform:Find("Container/bg_frane/bg_tab/btn_tabtype_7/text2"):GetComponent("UILabel").text = "moba私聊"
	SetClickCallback(bg_tab[ChatMsg_pb.chanel_MobaPrivate].gameObject, function(go)
		--curChanel = ChatMsg_pb.chanel_MobaPrivate
		--UpdateChatContent(ChatMsg_pb.chanel_MobaPrivate)
		UpdatePrivateContent()
	end)
	...
	bg_top.tittle.text = TextMgr:GetText("maincity_ui5")
	...
	GuildMobaChatData.AddListener(NotifyChanelChat)
	--GuildMobaChatData.AddListener(UpdateTabInfo)
...
print("chat start")
	...
	local lockLevel = 0--tonumber(TableMgr:GetGlobalData(DataEnum.ScGlobalDataId.ChatUnlock).value)
	...
	--[[if UnionInfoData.HasUnion() then
		bg_tab[ChatMsg_pb.chanel_GuildMobaTeam].transform:GetComponent("UIToggle").enabled = true
	else
		bg_tab[ChatMsg_pb.chanel_GuildMobaTeam].transform:GetComponent("UIToggle").enabled = false
	end]]
	bg_tab[ChatMsg_pb.chanel_GuildMobaTeam].transform:GetComponent("UIToggle").enabled = true
	
	UpdateTabRedPoint()
	GuildWarMain.SetChatPreviewRedPoint(false)
	TranslateChatContent()
	
	local chatTab = Global.GuildMobaGetChatEnterChanel()
	if privateChat ~= nil then
		PrivateChat(privateChat.name , privateChat.charid , privateChat.banner , privateChat.offid, privateChat.guildOfficialId)
		bg_tab[ChatMsg_pb.chanel_MobaPrivate].transform:GetComponent("UIToggle"):Set(true)
		
	elseif privateShare ~= nil or chatTab == ChatMsg_pb.chanel_MobaPrivate or chatTab == ChatMsg_pb.chanel_discussiongroup then
		UpdatePrivateContent()
		bg_tab[ChatMsg_pb.chanel_MobaPrivate].transform:GetComponent("UIToggle"):Set(true)
	else
...
function Close()
	GuildMobaChatData.SaveChatList()
	GuildWarMain.PreviewChanelChange()
	GuildWarMain.SetChatPreviewRedPoint(false)
	bg_tab = nil
	bg_bottom = nil 
	bg_mid = nil
	...
	GuildMobaChatData.RemoveListener(NotifyChanelChat)
