module("MobaMain", package.seeall)
...
local function UpdateActionList()
  --  print("UpdateActionList  ")
    ...
    actionCountLabel.text = string.format("%d/%d", MobaActionListData.GetValidCount(), tonumber(coreData.Value) + numTech)
...
--[[
	...
	_ui.lbTime = transform:Find("Container/bg_time/time"):GetComponent("UILabel")
	...
	_ui.btnShop = transform:Find("Container/bg_zhankai/Panel_left/bg_left/bg_shop/btn_bag"):GetComponent("UIButton")
	SetClickCallback(_ui.btnShop.gameObject, function(go)
		MobaStore.Show()
	end)	
	...
	ChatMenu.previewTog = {}
	for i=ChatMsg_pb.chanel_MobaPrivate , ChatMsg_pb.chanel_MobaTeam , 1 do
	...
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaWorld] = {}
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaWorld].key = ChatMsg_pb.chanel_MobaWorld
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaWorld].next = ChatMsg_pb.chanel_MobaTeam
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaTeam] = {}
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaTeam].key = ChatMsg_pb.chanel_MobaTeam
	--ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaTeam].next = ChatMsg_pb.chanel_MobaPrivate
	ChatMenu.previewTogKey[ChatMsg_pb.chanel_MobaTeam].last = ChatMsg_pb.chanel_MobaWorld
	...
	_ui.refreshtime = transform:Find("Container/bg_time/time"):GetComponent("UILabel")
	_ui.states = {}
	
	for i=1,4 do 
	  _ui.states[i] = {}
	  _ui.states[i].btn = transform:Find("Container/bg_time/bg_stage/stage"..i):GetComponent("UISprite")
	  _ui.states[i].shanshuo = transform:Find("Container/bg_time/bg_stage/stage"..i.."/shanshuo")
	  _ui.states[i].shanshuoyici = transform:Find("Container/bg_time/bg_stage/stage"..i.."/shanshuoyici")
	...
	_ui.StateText1 = transform:Find("Container/bg_time/bg_stage/jieduan/midll/base/stage1"):GetComponent("UILabel")
	_ui.StateText2 = transform:Find("Container/bg_time/bg_stage/jieduan/midll/base/stage1 (1)"):GetComponent("UILabel")
	_ui.jieduan = transform:Find("Container/bg_time/bg_stage/jieduan")
	_ui.jieduan.gameObject:SetActive(false)
    ...
    if MobaData.GetMobaEnterInfo() ~= nil then
        showMobaTime()
    else
        MobaData.RequestMobaEnter(showMobaTime)
    end
...
function UpdateMobaState()
	...
	_ui.statecoroutine = coroutine.start(function()
		if _ui == nil then 
			return 
		end 
		local state = MobaData.GetMobaState()
		_ui.StateText1.text =  String.Format(TextMgr:GetText("ui_moba_139"), state)
		_ui.StateText2.text =  String.Format(TextMgr:GetText("ui_moba_139"), state)
		_ui.jieduan.gameObject:SetActive(true)
		coroutine.wait(2)
		_ui.jieduan.gameObject:SetActive(false)
		for i=1,4 do 
		  if state == i then 
...
local function UpdateMenuNotice()
	local hasMailNotice = MailListData.HasMobaNotice()
	if noticeList then
	print(hasMailNotice)
...
function Close()
    ...
    MailListData.ClearMobaMail()
	coroutine.stop(_ui.countdowncoroutine)
	MobaData.SetMobaState(-1)
    MainCityUI.DestroyTerrain()
	MobaChatData.ResetData()
	mapNet = nil
	noticeList = nil
	ChatMenu = nil
    _ui = nil
...
function Update()
		--拉取聊天记录
	if Serclimax.GameTime.GetSecTime() >= MobaChatData.GetNextTime() then
		if not requestChat then 
			requestChat = true
			MobaChatData.RequestChat(nil , function() 
				requestChat = false 
			end)
...
function PreviewChanelChange(dir, uiChat)
	...
	--if not Main.Instance:IsInBattleState() then
		...
		local curChannel = Global.MobaGetChatEnterChanel()
		...
		local hasNotice_guildChanel = MobaChatData.GetUnreadGuildCount() > 0
		...
		if curTog ~= nil then
			...
			Global.SetMobaChatEnterChanel(curTog)
		end
	--end
end
...
function UpdateChatHint(chanel, hintCount, uiChat)
	...
	local recentChat = MobaChatData.GetRecentNewChat(chanel ,hintCount)
