module("PVP_LuckyRotaryReward", package.seeall)
...
local function LoadUI()
    ...
    if _ui.status == nil then
        _ui.hintATKRoot.gameObject:SetActive(false)
        _ui.rewardButton.gameObject:SetActive(false)
    else
        _ui.hintATKRoot.gameObject:SetActive(true)
        _ui.rewardButton.gameObject:SetActive(true)
        ...
        elseif _ui.status == 2 then
            _ui.rewardLabel.text = TextMgr:GetText("mission_reward")
            SetClickCallback(_ui.rewardButton.gameObject, function(go)
                local req = ActivityMsg_pb.MsgWarLossRewardRequest()
                req.count = _ui.count
                Global.Request(Category_pb.Activity, ActivityMsg_pb.ActivityTypeId.MsgWarLossRewardRequest, req, ActivityMsg_pb.MsgWarLossRewardResponse, function(msg)
