...
function RequestData(cb)
	local req = ClientMsg_pb.MsgGetBuffRequest()
	Global.Request(Category_pb.Client, ClientMsg_pb.ClientTypeId.MsgGetBuffRequest, req, ClientMsg_pb.MsgGetBuffResponse, function(msg)
		...
		table.foreach(buffListData ,function(i,v) 
			if SLGBuffBonusTable[v.buffId] ~= nil then
			   for i =1,#SLGBuffBonusTable[v.buffId].bouns,1 do
				...
				if Global.CheckBuff(v.buffId) then
					table.insert(bouns, b)
				end
