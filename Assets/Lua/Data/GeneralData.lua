module("GeneralData", package.seeall)
...
function HasGeneralByBaseUID(uid)
    return generalsByUID[uid] ~= nil
end
...
function CanGradeUp(heroInfo, heroGradeUpData , ignoreLevel)
    if not heroGradeUpData then
        heroGradeUpData = TableMgr:GetHeroGradeUpDataByHeroIdGrade(heroInfo.baseid, heroInfo.heroGrade)
    end
    
	local conditionItem = true
	local conditionLevel = (heroInfo.level >= heroGradeUpData.unlockLevel)
	...
	return ignoreLevel and (conditionItem) or (conditionItem and conditionLevel)
...
function IsOutForExpediation(uid)
    return ArmySetoutData.HasHeroSetout(uid)
end
...
function GetPower(heroInfo)
    ...
    local eqs = {}
    for i =1,6 do 
        eqs[i] = HeroEquipData.GetCurEquipByPos(i,heroInfo)
        if eqs[i] ~= nil then
            power = power +eqs[i].EquipData.Fight
        end
    end 

    local suits = HeroEquipData.CollectHeroEquipSuitList(heroInfo)
    for i, v in pairs(suits) do
        local suit_data_set = TableMgr:GetSuitData(i)
        if suit_data_set ~= nil then
            for j, k in pairs(suit_data_set) do
                if v >= j then
                    for m = 1,#k.Bonus do
                        local attributeID = Global.GetAttributeLongID(k.Bonus[m].BonusType, k.Bonus[m].Attype)
                        ...
                        power = power +k.Bonus[m].Value*coef
                    end
                end
            end
        end
    end     
    return GetAttributes(heroInfo, 100)[0] + math.floor(power+0.5)
end
...
local function GetHeroEquipCacheKey(heroInfo)
    local eqs = {}
    for i =1,6 do 
        eqs[i] = HeroEquipData.GetCurEquipByPos(i,heroInfo)
    end 

    local cacheKey = string.format("%d;%d;%d;%d;%d;%d",
    eqs[1] ~= nil and eqs[1].data.uniqueid or 0,
    eqs[2] ~= nil and eqs[2].data.uniqueid or 0,
    eqs[3] ~= nil and eqs[3].data.uniqueid or 0,
    eqs[4] ~= nil and eqs[4].data.uniqueid or 0,
    eqs[5] ~= nil and eqs[5].data.uniqueid or 0,
    eqs[6] ~= nil and eqs[6].data.uniqueid or 0)
    return cacheKey
end
...
function GetAttributeFromHeroEquip(heroInfo, heroData)

    local activeCondition = 2
    local eqs = {}
    for i =1,6 do 
        eqs[i] = HeroEquipData.GetCurEquipByPos(i,heroInfo)
    end 
    ...
    if not attributesCache.fromHeroEquip[cacheKey] then
        local attributes = { [1] = {}, [2] = {}, [3] = {} }
        for i=1,6 do
            if eqs[i] ~= nil then
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy1,eqs[i].EquipData.AdditionAttr1,eqs[i].EquipData.Value1)
                local attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy1,eqs[i].EquipData.AdditionAttr1 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value1
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value1
                end
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy2,eqs[i].EquipData.AdditionAttr2,eqs[i].EquipData.Value2)
                attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy2,eqs[i].EquipData.AdditionAttr2 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value2
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value2
                end
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy3,eqs[i].EquipData.AdditionAttr3,eqs[i].EquipData.Value3)
                attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy3,eqs[i].EquipData.AdditionAttr3 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value3
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value3
                end 
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy4,eqs[i].EquipData.AdditionAttr4,eqs[i].EquipData.Value4)
                attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy4,eqs[i].EquipData.AdditionAttr4 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value4
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value4
                end 
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy5,eqs[i].EquipData.AdditionAttr5,eqs[i].EquipData.Value5)
                attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy5,eqs[i].EquipData.AdditionAttr5 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value5
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value5
                end  
                --print("22222222222222222222",eqs[i].EquipData.AdditionArmy6,eqs[i].EquipData.AdditionAttr6,eqs[i].EquipData.Value6)
                attributeID = Global.GetAttributeLongID(eqs[i].EquipData.AdditionArmy6,eqs[i].EquipData.AdditionAttr6 )
                if not attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = eqs[i].EquipData.Value6
                else
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + eqs[i].EquipData.Value6
                end  
            end
        end
                ...
                for j, k in pairs(suit_data_set) do
                    if v >= j then
                        for m = 1,#k.Bonus do
                            local attributeID = Global.GetAttributeLongID(k.Bonus[m].BonusType, k.Bonus[m].Attype)
                            if not attributes[activeCondition][attributeID] then
                                attributes[activeCondition][attributeID] = k.Bonus[m].Value
                            else
                                attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + k.Bonus[m].Value
                            end  
                        end
                    end
                end
        ...
        attributesCache.fromHeroEquip[cacheKey] = attributes

        return attributes
    end

    return attributesCache.fromHeroEquip[cacheKey]
end

function GetAttributes(heroInfo, v1, v2)
    
    local cacheKey = string.format("%d;%d;%d;%d;%d;%s|%s", 
    heroInfo.baseid, heroInfo.level, heroInfo.star, heroInfo.grade, heroInfo.heroGrade,
     GetPassiveSkillCacheKey(heroInfo.skill.passiveSkill),GetHeroEquipCacheKey(heroInfo))
     --print("GetAttributesGetAttributesGetAttributesGetAttributes",heroInfo.baseid,cacheKey)
    if not attributesCache.all[cacheKey] then
        ...
        local attributes_fromHeroEquips = GetAttributeFromHeroEquip(heroInfo)
        ...
        for activeCondition = 0, 3 do
            ...
            for attributeID, value in pairs(attributes_fromHeroEquips[activeCondition] or {}) do
                if attributes[activeCondition][attributeID] then
                    attributes[activeCondition][attributeID] = attributes[activeCondition][attributeID] + value
                else
                    attributes[activeCondition][attributeID] = value
...
function CalAttributeBonus()
    ...
    for _, heroInfo in ipairs(generals) do
        ...
        if buildingType ~= 0 then
            ...
            local attributes = GetAttributes(heroInfo)[1]
            for i = 1, 3 do
    ...
    if sortingConfig then
        if not compareFunctions[sortingConfig] then
            compareFunctions[sortingConfig] = function(heroInfo1, heroInfo2)
                ...
                local attributes1 = GetAttributes(heroInfo1)[activeCondition]
                local attributes2 = GetAttributes(heroInfo2)[activeCondition]
