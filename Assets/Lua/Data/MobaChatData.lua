module("MobaChatData", package.seeall)
...
function UpdateNewData(charInfo , auto , transEnable , chatReqInit , flag)
	...
	for _ , v in ipairs(charInfo) do
		if v.chanel == ChatMsg_pb.chanel_MobaWorld then --moba战场
		...
		elseif v.chanel == ChatMsg_pb.chanel_MobaTeam then -- moba阵营
	...
	if totalChat[tonumber(ChatMsg_pb.chanel_MobaWorld)] == nil then
		totalChat[tonumber(ChatMsg_pb.chanel_MobaWorld)] = nationChatList
	end
	...
	if totalChat[tonumber(ChatMsg_pb.chanel_MobaTeam)] == nil then
		totalChat[tonumber(ChatMsg_pb.chanel_MobaTeam)] = unionChatList
	end	
	...
	for _, v in ipairs(charInfo) do
		...
		elseif v.chanel == ChatMsg_pb.chanel_MobaTeam then
			table.insert(totalChat[300] , v)
			if #totalChat[300] > 2 then
				table.remove(totalChat[300],1)
			end
		elseif v.chanel == ChatMsg_pb.chanel_MobaWorld then
...
function GetRecentNewChat(chanel , n)
	...
	if chanel == ChatMsg_pb.chanel_MobaPrivate then
		newIndex = 200
	elseif chanel == ChatMsg_pb.chanel_MobaTeam then
		newIndex = 300
	end
	if totalChat ~= nil then
...
function RequestChatInfo()
	...
	req.init = true
	req.languagecode = Global.GGUIMgr:GetSystemLanguage() --Global.GTextMgr:GetCurrentLanguageID()
	req.chanel = ChatMsg_pb.chanel_Moba
...
function RequestChat(chanel , callback)
	...
	req.init = false
	req.languagecode = Global.GGUIMgr:GetSystemLanguage()--Global.GTextMgr:GetCurrentLanguageID()
	req.chanel = chanel == nil and ChatMsg_pb.chanel_Moba or chanel
