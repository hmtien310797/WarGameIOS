	...
	static int _CreateUnityEngine_WWW(IntPtr L)
	{
		try
		{
			...
			if (count == 1)
			{
				...
				UnityWebRequest obj = new UnityWebRequest(arg0);
				obj.downloadHandler = new DownloadHandlerBuffer();
				obj.SendWebRequest();
			...
			else if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(string), typeof(byte[])))
			{
				...
				UnityWebRequest obj = new UnityWebRequest(arg0, UnityWebRequest.kHttpVerbPOST);
				obj.uploadHandler = new UploadHandlerRaw(arg1);
				obj.downloadHandler = new DownloadHandlerBuffer();
				obj.SendWebRequest();
				ToLua.PushObject(L, obj);
				return 1;
			...
			else if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(string), typeof(UnityEngine.WWWForm)))
			{
				...
				UnityWebRequest obj = UnityWebRequest.Post(arg0, arg1);
				obj.downloadHandler = new DownloadHandlerBuffer();
				obj.SendWebRequest();
				ToLua.PushObject(L, obj);
				return 1;
			...
			else if (count == 3 && TypeChecker.CheckTypes(L, 1, typeof(string), typeof(byte[]), typeof(System.Collections.Generic.Dictionary<string,string>)))
			{
				...
				UnityWebRequest obj = new UnityWebRequest(arg0, UnityWebRequest.kHttpVerbPOST);
				if (arg1 is byte[] postData)
				{
					obj.uploadHandler = new UploadHandlerRaw(postData);
				}
				obj.downloadHandler = new DownloadHandlerBuffer();
				if (arg2 is System.Collections.Generic.Dictionary<string, string> headers)
				{
