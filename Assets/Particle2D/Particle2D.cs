...
public class Particle2D : UIWidget {
	...
	public enum Emitter { POINT, CIRCLE, RECT }
	...
	float GetTime () {
#if UNITY_EDITOR
		return (Application.isPlaying) ? (mTimeScale ? Time.unscaledTime : Time.time) : (now_unix_time() - startUnixTime) * 0.001f;
#else
		return mTimeScale ? Time.unscaledTime : Time.time;
#endif // UNITY_EDITOR
	}
	...
	float GetDeltaTime () {
		...
		if (Application.isPlaying) {
			dt = mTimeScale ? Time.unscaledDeltaTime : Time.deltaTime;
		} else {
		...
		return dt;
#else
		return mTimeScale ? Time.unscaledDeltaTime : Time.deltaTime;
#endif // UNITY_EDITOR
	}
	...
	void OnDrawGizmos () {
		if (isVisible && NGUITools.GetActive(this)) {
			...
			} else if (emitter == Emitter.CIRCLE) {
	...
	void SimpleFill ( BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Rect outer, Rect inner, ParticleUnit unit ) {
		// Set scale
