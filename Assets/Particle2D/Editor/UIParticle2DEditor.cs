...
public class UIParticle2DEditor : UIWidgetInspector
{
	...
	protected override void DrawCustomProperties () {
		...
		if (isSpriteListFoldout = EditorGUILayout.Foldout(isSpriteListFoldout, "Sprites", EditorStyles.foldout)) {
			...
			// Color
			GUILayout.Space(3f);
			//NGUIEditorTools.DrawProperty("TimeScale", serializedObject, "mTimeScale", GUILayout.Width(20f));

		}
		
		bool timeScale = EditorGUILayout.Toggle("TimeScale", obj.mTimeScale);
		if (obj.mTimeScale != timeScale) {
			NGUIEditorTools.RegisterUndo("TimeScale Change", target);
			obj.mTimeScale = timeScale;
		}
