module("TileInfo", package.seeall)
...
function GetSuggestedPower(sEntryData)
    ...
    if sEntryData then
		...
		elseif sEntryData.data.entryType == Common_pb.SceneEntryType_WorldMonster then
			return tableData_tMonsterRule.data[sEntryData.monster.level].referenceFight
...
local WorldCity_Reward = {
	[1] = {click_func = function(buff_ids)
        local tip_data =
        {
            buffTitle = TextMgr:GetText("ui_citybattle_24") , 
            buffDes = TextMgr:GetText("ui_citybattle_25") , 
            actList = buff_ids, 
            actListCount = nil,
            buffTip = true
        }        
...
local Stronghold_Reward = {
    ...
    [5] = {click_func = function(item)
        local tip_data =
        {
            buffTitle = TextMgr:GetText("SoliderUpgrade_ui6") , 
            buffDes = TextMgr:GetText("SoliderUpgrade_ui7") , 
            actList = item.actList,  
            actListCount = nil,
            buffTip = false
        }        
...
local function BeginSpy(_target, _param)
...
function Update()
    ...
    elseif entryType == Common_pb.SceneEntryType_Monster or entryType == Common_pb.SceneEntryType_WorldMonster then
...
local function GetMapGradeData(x, y)
    ...
    for i = #gradeTable, 1, -1 do
        local gradeData = gradeTable[i]
        if InMapGrade(x, y, gradeData) then
            return gradeData
        end
    end
end
...
local function SetLandInfo(tileInfo, artSettingData)
    ...
    tileInfo.nameLabel.color = baseLevel >= gradeData.baseLevel and Color.white or Color.red
    tileInfo.nameBgSprite1.spriteName = gradeData.blockTitlebg
...
local function SetHomeInfo(tileInfo)
    ...
    infoList[1].transform.gameObject:SetActive(false)
    ...
    if homeMsg.charid == charId then
        ...
        --进入自己主基地
        buttonGroupList[buttonCount][1].label.text = TextMgr:GetText(Text.ui_worldmap_47)
    ...
    elseif selfGuildId ~= 0 and selfGuildId == guildMsg.guildid then
        ...
        SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
            ...
            --BattleMove.Show(Common_pb.TeamMoveType_ResTransport, uid, mapX, mapY, Hide)
        end)
        --盟友基地驻防
        buttonGroupList[buttonCount][2].label.text = TextMgr:GetText(Text.union_anagement6)
        SetClickCallback(buttonGroupList[buttonCount][2].button.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            
            Embassy.CanEmbassy(uid,tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY,homeMsg.charid)      
            --BattleMove.Show(Common_pb.TeamMoveType_Garrison, uid, mapX, mapY, Hide)
        end)
        --盟友基地更多
        ...
        SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
			BeginSpy(1, tonumber(tileMsg.home.homelvl))
        end)
        --攻击敌人基地
        buttonGroupList[buttonCount][2].label.text = TextMgr:GetText(Text.ui_worldmap_30)
        SetClickCallback(buttonGroupList[buttonCount][2].button.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            BattleMove.Show(Common_pb.TeamMoveType_AttackPlayer, uid, 0, tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
        end)
        --集结敌人基地
        buttonGroupList[buttonCount][3].label.text = TextMgr:GetText(Text.ui_worldmap_28)
        SetClickCallback(buttonGroupList[buttonCount][3].button.gameObject, function()
            -- MessageBox.Show(TextMgr:GetText(Text.common_ui1))
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            ...
            local x = mapX
            local y = mapY
            local target_level = tileMsg.home.homelvl >= tonumber(TableMgr:GetGlobalData(DataEnum.ScGlobalDataId.MassMinLevel).value) 
            local target_had_union = tileMsg.ownerguild.guildid ~= 0
            --print(target_level,tileMsg.home.homelvl,tonumber(TableMgr:GetGlobalData(DataEnum.ScGlobalDataId.MassMinLevel).value) ,target_had_union,target_level or target_had_union)
            mtc.target_enable_mass =  target_level or target_had_union
                    ...
                    assembled_time.Show(function(time)
                        if time ~= 0 then
                            ...
                            if building ~= nil then
                                ...
                                mtc:ShowCreateMassBattleMove(uid, tileMsg.home.name, x, y,curAssembledData.armynum,time , Common_pb.SceneEntryType_Home , nil)
                            end                           
                        end
                    end)
                end
            end)   
            --BattleMove.Show(Common_pb.TeamMoveType_Garrison, , Hide)
        end)
        --敌人基地更多
        buttonGroupList[buttonCount][4].label.text = TextMgr:GetText(Text.ui_worldmap_6)
        SetClickCallback(buttonGroupList[buttonCount][4].button.gameObject, function()
            --MessageBox.Show(TextMgr:GetText(Text.common_ui1))
            --Traget_Set.Show(homeMsg.name,mapX,mapY)
            local name = homeMsg.name
            ...
            tileInfoMore:Open(buttonGroupList[buttonCount][4].button.gameObject,name,mapX,mapY,Skin.GetSkinTextureNamePath(homeMsg.skin))
        end)
        --敌人基地查看
...
local function SetMonsterInfo(tileInfo)
    ...
    ShareCommon.LoadRewardList(_ui, tileInfo.rewardGrid.transform, dropId)
	...
	local isWorldMonst = monsterLevel == 1000
	tileInfo.monsterTitle.text = isWorldMonst and TextMgr:GetText("map_rebel_1000") or TextMgr:GetText("rebel_tero") 
	tileInfo.fleeLabel.transform.parent.gameObject:SetActive(not isWorldMonst)
	local worldMonstCont = buttonGroupList[buttonCount].info:Find("bg")
	print(buttonCount , monsterLevel)
	if worldMonstCont and isWorldMonst then
		worldMonstCont.gameObject:SetActive(true)
		--if monsterLevel == 1000 then
			print(MainData.GetData().dayWorldMonster)
			worldMonstCont:Find("text"):GetComponent("UILabel").text = System.String.Format(TextMgr:GetText("act_mst_hint1") , MainData.GetData().dayWorldMonster)
		--end
	end
    ...
    SetClickCallback(buttonGroupList[buttonCount][2].button.gameObject, function()
        ...
        elseif not RebelWantedData.IsLevelUnlocked(monsterLevel) then
            ...
            BattleMove.SetMonsterPower(tileInfo.suggestPower.text)
            BattleMove.Show(Common_pb.TeamMoveType_AttackMonster, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
        end
    end)
    --野怪更多
    ...
    if buttonGroupList[buttonCount][3] and buttonGroupList[buttonCount][4] then
		...
		SetClickCallback(buttonGroupList[buttonCount][3].button.gameObject, function()
			...
			elseif not RebelWantedData.IsLevelUnlocked(monsterLevel) then
                ...
                BattleMove.SetMonsterPower(tileInfo.suggestPower.text)
				BattleMove.Show(Common_pb.TeamMoveType_AttackMonster, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide ,nil, nil , nil , nil , sweepCount)
			end
		end)
...
local function LoadWorldCityBuff(grid , itemPrefab , dropdata)
	local info = NGUITools.AddChild(grid.gameObject , itemPrefab.gameObject)
	info.transform:SetParent(grid.transform , false)
	info.gameObject:SetActive(true)
	local buff = TableMgr:GetSlgBuffData(dropdata.contentId)
	...
	info.transform:Find("bg_num").gameObject:SetActive(false)
	SetClickCallback(info.gameObject,function()
		WorldCity_Reward[1].click_func({dropdata.contentId})
	end)
...
local function SetRebelInfo(tileInfo)
    ...
    --攻击活动野怪
    local stamina
    if monsterMsg.actMonster.finder == MainData.GetCharId() then
        stamina = RebelData.GetActivityInfo().actSta
    elseif monsterMsg.actMonster.finderGuildId == UnionInfoData.GetGuildId() then
            ...
            if success then
                assembled_time.Show(function(time)
                    if time ~= 0 then
                        ...
                        if building ~= nil then
                            local curAssembledData = TableMgr:GetAssembledData(building.data.level)
                            mtc:ShowCreateMassBattleMove(uid, TextMgr:GetText(tileData.name), x, y,curAssembledData.armynum,time)
                        end                           
                    end
                end)
            end
...
local function SetResourceInfo(tileInfo)
    ...
    if owner == 0 then
        ...
        SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
            ...
            if baseLevel < tileData.baseLevel then
                ...
                local uid = tileMsg ~= nil and tileMsg.data.uid or 0
                BattleMove.Show(Common_pb.TeamMoveType_ResTake, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
            end
        end)
        --无人资源更多
    ...
    elseif selfGuildId ~= 0 and selfGuildId == guildMsg.guildid then
        ...
        buttonGroupList[buttonCount][1].label.text = TextMgr:GetText(Text.ui_worldmap_29)
        SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
            BeginSpy(1, tonumber(tileMsg.res.ownerhomelvl))
        end)
        --攻击敌人采集的资源
        buttonGroupList[buttonCount][2].label.text = TextMgr:GetText(Text.ui_worldmap_30)
        SetClickCallback(buttonGroupList[buttonCount][2].button.gameObject, function()
...
local function SetBarrackInfo(tileInfo)
    ...
    elseif selfGuildId ~= 0 and selfGuildId == guildMsg.guildid then
        ...
        buttonGroupList[buttonCount][1].label.text = TextMgr:GetText(Text.ui_worldmap_29)
        SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
            BeginSpy(1, tonumber(barrackMsg.homelvl))
        end)
        --攻击敌人兵营
        buttonGroupList[buttonCount][2].label.text = TextMgr:GetText(Text.ui_worldmap_30)
        SetClickCallback(buttonGroupList[buttonCount][2].button.gameObject, function()
            ...
            BattleMove.Show(moveType, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
        end)
        --敌人兵营更多
    ...
    if selfGuildId == guildMsg.guildid then
        ...
        elseif buildingType == MapMsg_pb.GuildBuildTypeTrainHouse then
            ...
            if buildingMsg.hasSelfArmy then
                ...
                SetClickCallback(buttonGroupList[buttonCount][1].button.gameObject, function()
                    BattleMove.Show(Common_pb.TeamMoveType_TrainField, tileMsg.data.uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
                end)
            end
            --己方联盟训练场更多
            buttonGroupList[buttonCount][2].label.text = TextMgr:GetText(Text.ui_worldmap_29)
...
local function UpdateGovernmentInfo(tileInfo)
    ...
    if selfGuildId == guildid and selfGuildId ~= 0 then
        ...
        SetClickCallback(tileInfo.friendly_garrison.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            Embassy.CanGOVEmbassy(uid,mapX,mapY) 
        end)    
        
        SetClickCallback(tileInfo.friendly_check_garrison.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            Embassy.ShowGOVMode(uid,mapX,mapY,nil) 
        end)           
        ...
        SetClickCallback(tileInfo.hostile_attack.gameObject, function()
            ...
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            BattleMove.Show(Common_pb.TeamMoveType_AttackCenterBuild, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
        end)
                ...
                if success then
                    assembled_time.Show(function(time)
                        if time ~= 0 then
                            ...
                            if building ~= nil then
                                local curAssembledData = TableMgr:GetAssembledData(building.data.level)
                                mtc:ShowCreateMassBattleMove(uid, TextMgr:GetText("GOV_ui7"), x, y,curAssembledData.armynum,time)
                            end                           
                        end
                    end)
                end
...
-----------------------------------------------------------------------------
    ...
    if selfGuildId == guildid and selfGuildId ~= 0 then
        ...
        SetClickCallback(tileInfo.friendly_garrison.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            if tileMsg ~= nil and tileMsg.centerBuild.turret ~= nil and tileMsg.centerBuild.turret.subType ~= nil then
                Embassy.CanTurretEmbassy(uid,mapX,mapY,tileMsg.centerBuild.turret.subType)  
            end
        end)    
        ...
        SetClickCallback(tileInfo.friendly_check_garrison.gameObject, function()
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            if tileMsg ~= nil and tileMsg.centerBuild.turret ~= nil and tileMsg.centerBuild.turret.subType ~= nil then
                Embassy.ShowGOVMode(uid,mapX,mapY,tileMsg.centerBuild.turret.subType) 
            end           
                ...
                if success then
                    assembled_time.Show(function(time)
                        if time ~= 0 then
                            ...
                            if building ~= nil then
                                local curAssembledData = TableMgr:GetAssembledData(building.data.level)
                                mtc:ShowCreateMassBattleMove(uid, TextMgr:GetText(TableMgr:GetTurretDataByid(tileMsg.centerBuild.turret.subType).name), x, y,curAssembledData.armynum,time)
                            end                           
                        end
                    end)
                end
...
local function UpdateWorldCityInfo(tileInfo,build_id , cityInfo)
    local entryType = tileMsg.data.entryType
    local now = GameTime.GetSecTime()
    local endTime = -1 
    local build_data = TableMgr:GetWorldCityDataByID(build_id)
    title_name = TextMgr:GetText(build_data.Name) 
	
	tileInfo.finishTxt.gameObject:SetActive(false)
	...
	local listitem = ResourceLibrary.GetUIPrefab("CommonItem/list_item")
	if cityInfo.occupyReward ~= nil and cityInfo.occupyReward.items ~= nil then
		for i=1,#cityInfo.occupyReward.items do

			local dropdata = {}
			dropdata.contentId = cityInfo.occupyReward.items[i].id
			dropdata.contentNumber = cityInfo.occupyReward.items[i].num         

			LoadRewardItem(tileInfo.rewards_grid , listitem , dropdata)
		end
		
	end
	if cityInfo.occupyBuff ~= "" then
		local dropdata = {}
		dropdata.contentId = tonumber(cityInfo.occupyBuff)
		LoadWorldCityBuff(tileInfo.rewards_grid , listitem , dropdata)
	end
	
    tileInfo.rewards_grid:Reposition()
	
	if cityInfo.monsterArmy ~= nil and cityInfo.monsterArmy.army ~= nil and cityInfo.monsterArmy.army.army ~= nil then
		for i=1 , #cityInfo.monsterArmy.army.army do
			local soldierTrf = tileInfo.enemyTroops:GetChild(i-1)
			soldierTrf.gameObject:SetActive(true)
			
			local soldierId = cityInfo.monsterArmy.army.army[i].armyId
			local soldierLevel = cityInfo.monsterArmy.army.army[i].armyLevel
			local soldierCount = cityInfo.monsterArmy.army.army[i].num
			local soldierData = TableMgr:GetBarrackData(soldierId, soldierLevel)
			soldierTrf:GetComponent("UISprite").spriteName = "bg_item"..(soldierData.Grade + 1)
			soldierTrf:Find("enemy"):GetComponent("UITexture").mainTexture = ResourceLibrary:GetIcon("Icon/Unit/", soldierData.SoldierIcon)
			soldierTrf:Find("Label"):GetComponent("UILabel").text = soldierCount
			UIUtil.SetClickCallback(soldierTrf.gameObject, function(go)
				if go == _ui.tipObject then
					_ui.tipObject = nil
				else
					_ui.tipObject = go
	...
	tileInfo.enemyHero.gameObject:SetActive(false)
	if cityInfo.monsterArmy ~= nil and cityInfo.monsterArmy.hero ~= nil and cityInfo.monsterArmy.hero.heros ~= nil and #cityInfo.monsterArmy.hero.heros > 0 then
		tileInfo.enemyHero.gameObject:SetActive(true)
		local heroData = TableMgr:GetHeroData(cityInfo.monsterArmy.hero.heros[1].baseid)
		local heroLv = cityInfo.monsterArmy.hero.heros[1].level
		local heroStar = cityInfo.monsterArmy.hero.heros[1].star
		tileInfo.enemyHero:Find("head icon/outline"):GetComponent("UISprite").spriteName = "head"..heroData.quality
		tileInfo.enemyHero:Find("head icon"):GetComponent("UITexture").mainTexture = ResourceLibrary:GetIcon(ResourceLibrary.PATH_ICON .. "herohead/", heroData.icon)
		tileInfo.enemyHero:Find("level text"):GetComponent("UILabel").text = heroLv
		tileInfo.enemyHero:Find("star"):GetComponent("UISprite").width = heroStar * 30
		UIUtil.SetClickCallback(tileInfo.enemyHero:Find("head icon").gameObject, function(go)
			if go == _ui.tipObject then
				_ui.tipObject = nil
			else
				_ui.tipObject = go
	...
	local process = WorldCityData.GetWorldCityProcess(cityInfo , build_id)
	tileInfo.defence_progress.value = process
	tileInfo.defence_progressLabel.text = math.floor(process*100) .. "%"
	
	local unlockText = ""
	local occupied = cityInfo.occupied
	
	local unlockInfo = WorldCityData.IsUnlock(build_id)
	local unlock = unlockInfo.unlock
	local finishFront = unlockInfo.finishFront
	local overMilitaryRank = unlockInfo.overMilitaryRank
	local finishRebel = unlockInfo.finishRebel
	print(unlock , finishFront , overMilitaryRank , finishRebel)
	
	tileInfo.hostile_attack.gameObject:SetActive((not occupied) and unlock)
	tileInfo.unlockLabel.gameObject:SetActive(occupied or (not unlock))
	tileInfo.finishTxt.gameObject:SetActive(occupied and unlock)
	tileInfo.finishTxt.text = System.String.Format(TextMgr:GetText("ui_citybattle_17") , cityInfo.occupyUserNum and cityInfo.occupyUserNum or 0)
	tileInfo.enmeyWaceLabel.gameObject:SetActive(not occupied)
	tileInfo.enmeyWaceLabel.text = System.String.Format(TextMgr:GetText("ui_citybattle_4") , cityInfo.wave)
	tileInfo.cityJumpBtn.gameObject:SetActive(false)
	if not unlock then
		local unlockText = ""
		if (not finishFront) then
			local city = TableMgr:GetWorldCityDataByID(build_id)
			local frontCity = TableMgr:GetWorldCityDataByID(city.FrontCity)
			local frontCityName = frontCity and TextMgr:GetText(frontCity.Name) or "--"
			unlockText = System.String.Format(TextMgr:GetText("ui_citybattle_5") ,frontCityName)
			tileInfo.cityJumpBtn.gameObject:SetActive(true)
		elseif (not overMilitaryRank) then
			local militaryData = TableMgr:GetMilitaryRankData(build_data.MilitaryRank,0)
			unlockText = System.String.Format(TextMgr:GetText("ui_citybattle_6") ,TextMgr:GetText(militaryData.Name))
		elseif (not finishRebel) then
			unlockText = TextMgr:GetText("ui_citybattle_18")
		end
		tileInfo.unlockLabel.text = unlockText
	elseif occupied then
		tileInfo.unlockLabel.text = TextMgr:GetText("ui_citybattle_7")
	end

    SetClickCallback(tileInfo.hostile_attack.gameObject,function() 
		local uid = tileMsg ~= nil and tileMsg.data.uid or 0
        BattleMove.Show(Common_pb.TeamMoveType_AttackWorldCity, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide, nil, nil, nil, tileMsg.data.entryType)   
	end)
    SetClickCallback(tileInfo.hostile_share.gameObject,function() 
		tileInfoMore:Open(tileInfo.hostile_share.gameObject,TextMgr:GetText(build_data.Name),mapX,mapY,TableMgr:GetArtSettingData(tileGid).icon)
	end)      
    
    SetClickCallback(tileInfo.help.gameObject,function() 
        MapHelp.Open(2600, false, nil, false, true)
    end)
	
	SetClickCallback(tileInfo.cityJumpBtn.gameObject,function() 
        CityMap.Show()
		Hide()
    end)   
...
SetWorldCityInfo = function(tileInfo,cityId)
    if _ui == nil then
        return
    end
	WorldCityData.RequestCityInfo(cityId , function(cityInfo)
		UpdateWorldCityInfo(tileInfo,cityId , cityInfo)
	end)
end
...
local StrongholdFuncs = {
        ...
        friendly_garrison = function(entryType,subtype)
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            Embassy.CanStrongholdEmbassy(entryType,subtype,uid,mapX,mapY)         
        end,
        friendly_check_garrison = function(entryType,subtype)
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            Embassy.ShowStrongholdMode(uid,mapX,mapY,subtype,entryType)         
        end,
        ...
        hostile_attack = function(entryType,subtype)  
            ...
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            BattleMove.Show(Common_pb.TeamMoveType_AttackCenterBuild, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide, nil, nil, nil, tileMsg.data.entryType)         
        end,
                ...
                if success then
                    assembled_time.Show(function(time)
                        if time ~= 0 then
                            ...
                            if building ~= nil then
                                local curAssembledData = TableMgr:GetAssembledData(building.data.level)
                                mtc:ShowCreateMassBattleMove(uid, title_name, x, y,curAssembledData.armynum,time)
                            end                           
                        end
                    end)
                end
        ...
        soliderLevelUp = function(entryType,subtype,uid)
            
            local titleName =""
            local seUid = uid
            local soliderLevelUpCfg = ""
            local soliderNumCfg = ""
            if entryType == Common_pb.SceneEntryType_Stronghold then
                titleName = TextMgr:GetText(TableMgr:GetStrongholdRuleByID(subtype).name)
                soliderLevelUpCfg = TableMgr:GetStrongholdRuleByID(subtype).SoldierLevelup
                soliderNumCfg = TableMgr:GetStrongholdRuleByID(subtype).SoldierNum
            elseif entryType == Common_pb.SceneEntryType_Fortress then
                titleName = TextMgr:GetText(TableMgr:GetFortressRuleByID(subtype).name) 
                soliderLevelUpCfg = TableMgr:GetFortressRuleByID(subtype).SoldierLevelup
                soliderNumCfg = TableMgr:GetFortressRuleByID(subtype).SoldierNum 
            end
            
            SoliderUpgrade.Show(titleName,seUid,soliderLevelUpCfg,soliderNumCfg)
        end,  
    }

local function UpdateStrongholdInfo(tileInfo,subtype,entryType,uid)
    if _ui == nil then
        return
    end    
    local guildid
    ...
    local info
    local reward_click_param = {}
    local dontHadSoliderLevelUp = false
    ...
    if entryType == Common_pb.SceneEntryType_Stronghold then
        ...
        dontHadSoliderLevelUp = System.String.IsNullOrEmpty(TableMgr:GetStrongholdRuleByID(subtype).SoldierLevelup) or System.String.IsNullOrEmpty(TableMgr:GetStrongholdRuleByID(subtype).SoldierNum)
		...
		if stronghold_msg.extraReward~=nil then 
			for i=1,#stronghold_msg.extraReward.items do
	 
				local dropdata = {}
				dropdata.contentId = stronghold_msg.extraReward.items[i].id
				dropdata.contentNumber = stronghold_msg.extraReward.items[i].num         

				LoadRewardItem(tileInfo.guildrewards_stronghold_grid , listitem , dropdata)
			end
			tileInfo.guildrewards_stronghold_grid:Reposition()
		end 
    ...
    elseif entryType == Common_pb.SceneEntryType_Fortress then
        ...
        dontHadSoliderLevelUp = System.String.IsNullOrEmpty(TableMgr:GetFortressRuleByID(subtype).SoldierLevelup) or System.String.IsNullOrEmpty(TableMgr:GetFortressRuleByID(subtype).SoldierNum)
    ...
    if not dontHadSoliderLevelUp then
        if entryType == Common_pb.SceneEntryType_Stronghold then
            reward_click_param[5] = {}  
            reward_click_param[5].actList = {}
            reward_click_param[5].actListCount={}
            
            local _SoliderGroup = {}
            for v in string.gsplit(TableMgr:GetStrongholdRuleByID(subtype).SoldierLevelup, ";") do
            ...
            for i=1,#_SoliderGroup do
                local item_id = _SoliderGroup[i].id*10+_SoliderGroup[i].level
                table.insert(reward_click_param[5].actList,item_id)
                reward_click_param[5].actListCount[item_id] = 0
            end     
            tileInfo.rewards_stronghold_items[5].root.gameObject:SetActive(true)
            SetClickCallback(tileInfo.rewards_stronghold_items[5].root.gameObject,
            function()
                Stronghold_Reward[5].click_func(reward_click_param[5])
            end)  
        elseif entryType == Common_pb.SceneEntryType_Fortress then
            reward_click_param[5] = {}  
            reward_click_param[5].actList = {}
            ...
            local _SoliderGroup = {}
            for v in string.gsplit(TableMgr:GetFortressRuleByID(subtype).SoldierLevelup, ";") do
                local args = string.split(v, ":")
                local soliderTab ={}
                soliderTab.id = tonumber(args[1])
                soliderTab.level = tonumber(args[2])
            ...
            tileInfo.rewards_fortress_items[5].root.gameObject:SetActive(true)
            SetClickCallback(tileInfo.rewards_fortress_items[5].root.gameObject,
            function()
                Fortress_Reward[5].click_func(reward_click_param[5])
            end) 
        end
    else
        tileInfo.rewards_stronghold_items[5].root.gameObject:SetActive(false)
        tileInfo.rewards_fortress_items[5].root.gameObject:SetActive(false)
    end


    if not dontHadSoliderLevelUp and (strongholdState ~= 3 and strongholdState ~= 1)  then
        dontHadSoliderLevelUp = true
    end
    ...
    if endTime < 0 then
        ...
        CountDown.Instance:Add("Hold_State",endTime,CountDown.CountDownCallBack(function(t)
            ...
            if endTime+1 - GameTime.GetSecTime() <= 0 then
                CountDown.Instance:Remove("Hold_State")  
                SetStrongholdInfo(tileInfo,subtype,entryType,false,uid)               
            end			
        end))
    ...
    UIUtil.SetBtnEnable(tileInfo.soliderLevelUp ,"btn_2", "btn_4", not dontHadSoliderLevelUp)

    tileInfo.soliderLevelUp.gameObject:SetActive(not dontHadSoliderLevelUp)
    ...
    if selfGuildId == guildid and selfGuildId ~= 0 then
        ...
        SetClickCallback(tileInfo.soliderLevelUp.gameObject,function() 
            if not dontHadSoliderLevelUp then
                StrongholdFuncs.soliderLevelUp(entryType,subtype,uid)
            end
        end)      
    else
        tileInfo.power.gameObject:SetActive(false)
...
SetStrongholdInfo = function(tileInfo,subtype,entryType,addlister,uid)
    if entryType == Common_pb.SceneEntryType_Stronghold then
        if addlister then
            StrongholdData.AddHoldRulingListener(DisposeStrongholdPush) 
        end   
        StrongholdData.ReqStrongholdInfoData(subtype,function()
            UpdateStrongholdInfo(tileInfo,subtype,entryType,uid)
            tileInfo.transform.gameObject:SetActive(true);
        end)        
    elseif entryType == Common_pb.SceneEntryType_Fortress then
		if addlister then
...
LoadUI = function()
    ...
    elseif entryType == Common_pb.SceneEntryType_Stronghold then
        if tileMsg ~= nil and tileMsg.centerBuild.stronghold ~= nil and tileMsg.centerBuild.stronghold.subtype ~= nil then
            ...
            SetStrongholdInfo(tileInfo,tileMsg.centerBuild.stronghold.subtype,entryType,true,tileMsg.data.uid)
        end        
    elseif entryType == Common_pb.SceneEntryType_Fortress then
        if tileMsg ~= nil and tileMsg.centerBuild.fortress ~= nil and tileMsg.centerBuild.fortress.subtype ~= nil then
            tileInfo = _ui.stronghold
            SetStrongholdInfo(tileInfo,tileMsg.centerBuild.fortress.subtype,entryType,true,tileMsg.data.uid)
        end
	elseif entryType == Common_pb.SceneEntryType_WorldCity then
        if tileMsg ~= nil and tileMsg.worldCity ~= nil and tileMsg.worldCity.cityId ~= nil then
            tileInfo = _ui.cityInfo
            SetWorldCityInfo(tileInfo,tileMsg.worldCity.cityId)
        end    		
    end
    ...
    tileInfo.coordLabel.text = string.format("X:%d Y:%d", mapX, mapY)
...
local function MapHelpCallback()
    MapHelp.Open(401, false, TeleportClickCallback, false)
end

--空地传送
function TeleportClickCallback()
    local item = 0
    local transType = 0
    local buy = false
    ...
    local gradeData = GetMapGradeData(mapX, mapY)
    local baseLevel = BuildingData.GetCommandCenterData().level
    if baseLevel < gradeData.baseLevel then
    ...
    MessageBox.ShowConfirmation(WorldMap.IsInRestrictArea(mapX,mapY) and BuffData.HasShield(), TextMgr:GetText("ControlZone_ui2"), function() -- 管制区取消护盾提醒
        MessageBox.ShowConfirmation(#(ActionListData.GetData()) > 0, TextMgr:GetText("ui_maphint_1"), function() -- 行军队列不为空提醒
            if RadarData.IsPathToMe() then
...
function Awake()
    ...
    tileInfoMore = TileInfoMore(transform:Find("bg_more"))
    _ui.landMore = TileInfoMore(transform:Find("bg_more01"))
...
local function LoadPlayerInfo()
	local prefab = ResourceLibrary.GetUIPrefab("TileInfo/PlayerInfo")
    ...
    playerInfo.unionLabel = transform:Find("PlayerInfo/bg_info/bg_date/bg_union/txt_num"):GetComponent("UILabel")
    playerInfo.coordLabel = transform:Find("PlayerInfo/bg_info/bg_date/txt_num"):GetComponent("UILabel")
    playerInfo.powerLabel = transform:Find("PlayerInfo/bg_info/bg_date/bg_power/txt_num"):GetComponent("UILabel")
    ...
    playerInfo.skinButton.gameObject:SetActive(tileMsg.data.entryType == Common_pb.SceneEntryType_Home)
...
local function LoadBlockInfo()
	local prefab = ResourceLibrary.GetUIPrefab("TileInfo/BlockInfo")
    ...
    blockInfo.teleportButton = transform:Find("BlockInfo/bg_info/btn_1"):GetComponent("UIButton")
    blockInfo.baseButton = transform:Find("BlockInfo/bg_info/btn_2"):GetComponent("UIButton")
    ...
    SetClickCallback(blockInfo.baseButton.gameObject, function()
        MapHelp.Open(402, false, function()  
            local uid = tileMsg ~= nil and tileMsg.data.uid or 0
            BattleMove.Show(Common_pb.TeamMoveType_Camp, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
        end, false)        
    end)
    --占领空地
    SetClickCallback(blockInfo.occupyButton.gameObject,
        function() 
            ...
            function()
                ...
                elseif WorldMap.IsInRestrictArea(mapX, mapY) then
                ...
                elseif not WorldBorderData.IsSelfNeighboringBorder(mapX, mapY) then
                    ...
                    local uid = tileMsg ~= nil and tileMsg.data.uid or 0
                    BattleMove.Show(Common_pb.TeamMoveType_Occupy, uid, "", tileMsg ~= nil and tileMsg.data.pos.x or mapX,tileMsg ~= nil and tileMsg.data.pos.y or mapY, Hide)
                end
            end, false)
        end)
    --空地更多
    SetClickCallback(blockInfo.moreButton.gameObject, function()
        ...
        local artSettingData = TableMgr:GetArtSettingData(tileGid)
        --print(TextMgr:GetText(artSettingData.name),mapX,mapY)
        --Traget_Set.Show(TextMgr:GetText(artSettingData.name),mapX,mapY)
        --tileInfoMore:Open(blockInfo.moreButton.gameObject,TextMgr:GetText(artSettingData.name),mapX,mapY,artSettingData.icon)
        ...
        send.content = TextMgr:GetText("chatchat_coordinate") 
        send.spectext = mapX .. "," .. mapY .. "," .. artSettingData.icon .. "," .. "分享时玩家输入的一句话"
...
local function LoadWorldCityBuilding()
	local prefab = ResourceLibrary.GetUIPrefab("CityBattle/CityBuilding")
	NGUITools.AddChild(gameObject, prefab).name = "CityBuilding"
	local transform = transform

    local cityInfo = {}
    cityInfo.transform = transform:Find("CityBuilding")
    cityInfo.bg = transform:Find("CityBuilding/bg_info"):GetComponent("UISprite")
    cityInfo.bg_collider = transform:Find("CityBuilding/bg_info/bg")
    cityInfo.titleBg = transform:Find("CityBuilding/bg_info/bg_title"):GetComponent("UISprite")
    cityInfo.title = transform:Find("CityBuilding/bg_info/bg_title/txt_name"):GetComponent("UILabel")
	cityInfo.finishTxt = transform:Find("CityBuilding/bg_info/num"):GetComponent("UILabel")

    cityInfo.icon = transform:Find("CityBuilding/bg_info/bg_build/icon_castle"):GetComponent("UITexture")
    cityInfo.coordLabel = transform:Find("CityBuilding/bg_info/bg_date/coordinate"):GetComponent("UILabel")
   -- cityInfo.coordLabel = transform:Find("CityBuilding/bg_info/bg_date/icon_power/text_power"):GetComponent("UILabel")

    cityInfo.help = transform:Find("CityBuilding/bg_info/btn_help")
	cityInfo.unlockLabel = transform:Find("CityBuilding/bg_info/Label"):GetComponent("UILabel")
	
    cityInfo.defence = transform:Find("CityBuilding/bg_info/defence")
    cityInfo.defence_progress = transform:Find("CityBuilding/bg_info/defence/jindu"):GetComponent("UISlider")
    cityInfo.defence_progressLabel = transform:Find("CityBuilding/bg_info/defence/jindu/Label"):GetComponent("UILabel")
	
    cityInfo.rewards = transform:Find("CityBuilding/bg_info/bg_rewards")
    cityInfo.rewards_grid = transform:Find("CityBuilding/bg_info/bg_rewards/Stronghold/dailyrewards/Scroll View/Grid"):GetComponent("UIGrid")
	cityInfo.enemyBg = transform:Find("CityBuilding/bg_info/mid")
	cityInfo.enemyHero = transform:Find("CityBuilding/bg_info/mid/Stronghold/enemy/listitem_hero")
	cityInfo.enemyTroops = transform:Find("CityBuilding/bg_info/mid/Stronghold/enemy/troops")
	cityInfo.enmeyWaceLabel = transform:Find("CityBuilding/bg_info/mid/title"):GetComponent("UILabel")
	cityInfo.cityJumpBtn = transform:Find("CityBuilding/bg_info/btn_jump")
	
    transform:Find("CityBuilding/bg_info/bg_rewards/title").gameObject:SetActive(false)
    
    cityInfo.hostile = transform:Find("CityBuilding/bg_info/btn_2")
    cityInfo.hostile_share = transform:Find("CityBuilding/bg_info/btn_2/btn_2")
    cityInfo.hostile_attack = transform:Find("CityBuilding/bg_info/btn_2/btn_1")
 
    _ui.cityInfo = cityInfo 
    SetClickCallback(_ui.cityInfo.bg.gameObject, Hide)
end

local function LoadMonsterInfo()
	local prefab = ResourceLibrary.GetUIPrefab("TileInfo/MonsterInfo")
	...
	monsterInfo.monsterTitle = transform:Find("MonsterInfo/bg_info/bg_date/txt_tero"):GetComponent("UILabel")
...
function LoadStronghold()
    ...
    stronghold.soliderLevelUp = transform:Find("Stronghold/bg_info/btn_4/btn_5"):GetComponent("UIButton")
    ...
    stronghold.dailyrewards_stronghold_grid = transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/rewards/left/Scroll View/Grid"):GetComponent("UIGrid")
    stronghold.guildrewards_stronghold_grid = transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/rewards/right/Scroll View/Grid"):GetComponent("UIGrid")
    ...
    local icon_paths ={"icon","icon (1)","icon (2)","icon (4)","icon (5)"}
    for i=1,5 do
    ...
    local icon_paths ={"icon","icon (1)","icon (2)","icon (3)","icon (5)"}
    for i=1,5 do
    ...
    stronghold.info_fortress.daily = transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/dailyrewards")
	stronghold.info_fortress.rewards = transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/rewards")
	
	stronghold.info_fortress.daily.gameObject:SetActive(false)
	stronghold.info_fortress.rewards.gameObject:SetActive(true)
    ...
    transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/rewards/left/Scroll View"):GetComponent("UIPanel").depth = _ui.bgPanel.depth + 1
	transform:Find("Stronghold/bg_info/bg_rewards/Stronghold/rewards/right/Scroll View"):GetComponent("UIPanel").depth = _ui.bgPanel.depth + 1
...
function Show(msg, data, gid, x, y)
    if WorldMap.TESTGOV then
