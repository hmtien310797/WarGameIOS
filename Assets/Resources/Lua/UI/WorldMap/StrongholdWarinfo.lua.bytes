module("StrongholdWarinfo", package.seeall)
...
local function UpdateWarInfoItem(CenterBuildWarLog_msg, index)
    ...
    if CenterBuildWarLog_msg.logId == 30040 then
        _ui.warInfos[index].text.gameObject:SetActive(false)
        _ui.warInfos[index].no_one_occupied.gameObject:SetActive(true)
        ...
        if StrongholdData.GetContendEndTime(CenterBuildWarLog_msg.subType) > Serclimax.GameTime.GetSecTime() then
            ...
            CountDown.Instance:Add("StrongholdWarinfo" .. index, StrongholdData.GetContendEndTime(CenterBuildWarLog_msg.subType), CountDown.CountDownCallBack(function(t)
                local endtime = StrongholdData.GetContendEndTime(CenterBuildWarLog_msg.subType)
                _ui.warInfos[index].time.text = t
                local time_value =  TableMgr:GetGlobalData(100140)
                ...
                --_ui.warInfos[index].time_progress.value = 1 - (endtime - Serclimax.GameTime.GetSecTime()) / (8*3600)
                if endtime - Serclimax.GameTime.GetSecTime() <= 0 then
                    CountDown.Instance:Remove("StrongholdWarinfo" .. index)
    ...
    _ui.warInfos[index].name.text = TextMgr:GetText(TableMgr:GetStrongholdRuleByID(CenterBuildWarLog_msg.subType).name)
    local stronghold_msg = StrongholdData.GetStrongholdData(CenterBuildWarLog_msg.subType)
    _ui.warInfos[index].des .text = TextMgr:GetText(stronghold_msg.buildDesc)
    _ui.warInfos[index].tex.maintexture = ResourceLibrary:GetIcon(ResourceLibrary.PATH_ICON .. "WorldMap/", stronghold_msg.buildIcon)
end
...
local function InitWarInfo(i)
    ...
    _ui.warInfos[i].des = _ui.warInfos[i].trf:Find("des"):GetComponent("UILabel")
    _ui.warInfos[i].tex = _ui.warInfos[i].trf:Find("Texture"):GetComponent("UITexture")
...
LoadUI = function()
    _ui.log = StrongholdData.GetStrongholdWarLogInfoMsg()
    ...
    UpdateWarInfo()
    CountDown.Instance:Add("StrongholdWarinfo0", StrongholdData.GetAllEndTime(), CountDown.CountDownCallBack(function(t)
        _ui.totleTime.text = t
        if StrongholdData.GetAllEndTime() - Serclimax.GameTime.GetSecTime() <= 0 then
            CountDown.Instance:Remove("StrongholdWarinfo0")
...
function  Awake()
    ...
    LoadUI()
    StrongholdData.AddStateListener(LoadUI)
end

function Show()    
    StrongholdData.ReqStrongholdWarLogInfo(function()
        Global.OpenUI(_M)
    end)
end
...
function Close()   
    for i = 0, #_ui.log.warLogs do
        CountDown.Instance:Remove("StrongholdWarinfo" .. i)
    end
    StrongholdData.RemoveStateListener(LoadUI)
    _ui = nil
end
