...
local UserTitle = {"rank_ui8","rank_ui12","rank_ui13","rank_ui9","rank_ui10","rank_ui11","rank_ui14","rank_ui16"}
...
local function UserRankOverviewRequest(callback)
	...
	local req = ClientMsg_pb.MsgUserRankOverviewRequest()
    Global.Request(Category_pb.Client, ClientMsg_pb.ClientTypeId.MsgUserRankOverviewRequest, req, ClientMsg_pb.MsgUserRankOverviewResponse, function(msg)
...
local function MakeUserEndlessList(scroll, data, pos)
    _ui.searchRank = data.myRank.rank
...
local function MakeUserRank(data, ismy)
	...
	if data.myRank == nil or data.myRank.rank == 0 or data.myRank.rank > 100 then
		...
		if data.myRank == nil or data.myRank.rank == 0 then
			...
			if data.beforeRank ~= nil and data.beforeRank.rank ~= 0 then
				MakeUserRank(data.beforeRank, false)
			end
			MakeUserRank(data.myRank, true)
			if data.afterRank ~= nil and data.afterRank.rank ~= 0 then
				MakeUserRank(data.afterRank, false)
			end
			_ui.container2.mid02_mygrid:Reposition()
		end
		MakeUserEndlessList(_ui.container2.mid02_scroll, data, _ui.container2.mid02_scroll_pos)
	else
		_ui.container2.mid02:SetActive(false)
		_ui.container2.mid03:SetActive(true)
		MakeUserEndlessList(_ui.container2.mid03_scroll, data, _ui.container2.mid03_scroll_pos)
	end
end
