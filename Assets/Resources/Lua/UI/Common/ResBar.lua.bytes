...
local function NewEnergyTipCallback()
  --[[  if  moduleName == "Barrack" then
        MainCityUI.UseResItem(25,function() Barrack.Open3DArea() end)
        Barrack.Close3DArea()
    else
        MainCityUI.UseResItem(25)
    end  
    --]]
    MainCityUI.CheckAndBuyEnergy(false)
    if MainData.GetEnergy() >= MainData.GetMaxEnergy() then
        StopEnergyTipCoroutine()
    else
        StartEnergyTipCoroutine()
...
local configList = 
{
    ...
    food = {icon = "3", tipCallback = function() 
        if  moduleName == "Barrack" then
            ...
            MainCityUI.UseResItem(3,function() Barrack.Open3DArea() end)
            Barrack.Close3DArea()
        else
            MainCityUI.UseResItem(3)
        end
        return false 
    end},
    steel = {icon = "4", tipCallback = function() 
        if  moduleName == "Barrack" then
            ...
            MainCityUI.UseResItem(4,function() Barrack.Open3DArea() end)
            Barrack.Close3DArea()
        else
            MainCityUI.UseResItem(4)
        end        
        return false 
    end},
    oil = {icon = "5", tipCallback = function()
        if  moduleName == "Barrack" then
            MainCityUI.UseResItem(5,function() Barrack.Open3DArea() end)
            Barrack.Close3DArea()
        else
            MainCityUI.UseResItem(5)
        end        
        return false 
    end},
    elec = {icon = "6", tipCallback = function() 
        if  moduleName == "Barrack" then
            MainCityUI.UseResItem(6,function() Barrack.Open3DArea() end)
            Barrack.Close3DArea()
        else
            MainCityUI.UseResItem(6)
        end        
        return false 
    end},
}
...
local function LoadRes()
    if resList == nil then
        ...
        local showLabel = NewResBarLableList--showLabelList[moduleName] --
        
        for i, v in ipairs(showLabel) do
            if CheckUnLock(v) then
                local child = grid:GetChild(i - 1)
                if child == nil then
                    child = NGUITools.AddChild(grid.gameObject, resGameObject).transform
                end
                resList[v] = {}
                resList[v].bg = child:Find("bg_num")
                resList[v].label = child:Find("bg_num/num"):GetComponent("UILabel")
                resList[v].icon = child:Find("bg_num/icon"):GetComponent("UITexture")
                local icon
                if string.starts(v, "item") then
                    local itemId = string.sub(v, 5)
                    local itemData = TableMgr:GetItemData(itemId)
                    icon = itemData.icon
                else
                    icon = configList[v].icon
                end
                resList[v].icon.mainTexture = ResourceLibrary:GetIcon("Item/", icon)
                resList[v].btnAdd = child:Find("bg_num/btn_add"):GetComponent("UIButton")
                resList[v].btnResinfo = child:Find("bg_num/icon").gameObject
                local addVisible = configList[v] ~= nil and configList[v].addVisible
                if addVisible then
                    SetClickCallback(resList[v].btnAdd.gameObject, configList[v].addCallback)
                end
            end
        end
        for i = 4, #showLabel + 1 do
            local child = grid:GetChild(i - 1)
            if child ~= nil then
...
local function PreviewChanelChange(dir)
	...
	MainCityUI.PreviewChanelChange(dir, ChatMenu)
end
...
InitCharMenu = function()
	...
	ChatMenu.previewTog[ChatMsg_pb.chanel_private].gameObject:SetActive(true)
	
	SetClickCallback(ChatMenu.chatBtn.gameObject, function(go)
		if moduleName == "Barrack" then
            Chat.CloseCallBack = function() Barrack.Open3DArea() end
            Barrack.Close3DArea()
        end    
...
local function UpdateChatHint(chanel, hintCount)
	if not Main.Instance:IsInBattleState() then
		...
		local recentChat = ChatData.GetRecentNewChat(nil, 2)
...
function Init()
    ...
    MainCityUI.AddCommonItemBagListener(UpdateElse)
...
function Close()
    ...
    MainCityUI.RemoveCommonItemBagListener(UpdateElse)
