module("MobaWallHero", package.seeall)
...
local interface={
    ["SetArmyTeamRequest"] = MobaMsg_pb.MsgMobaSetArmyTeamRequest,
    ["SetArmyTeamResponse"] = MobaMsg_pb.MsgMobaSetArmyTeamResponse,
    ["SetArmyTeamRequestTypeID"] = MobaMsg_pb.MobaTypeId.MsgMobaSetArmyTeamRequest,
    ["Category_pb"] = Category_pb.Moba,
}

local interface_guild={
    ["SetArmyTeamRequest"] = GuildMobaMsg_pb.GuildMobaSetArmyTeamRequest,
    ["SetArmyTeamResponse"] = GuildMobaMsg_pb.GuildMobaSetArmyTeamResponse,
    ["SetArmyTeamRequestTypeID"] = GuildMobaMsg_pb.GuildMobaTypeId.GuildMobaSetArmyTeamRequest,
...
local function ConfirmClickCallback(go)
   ...
   -- SelectArmy.Show(teamType)
    local req = GetInterface("SetArmyTeamRequest")()
    req.data.team:add()
    req.data.team[1] = MobaTeamData.GetDataByTeamType(teamType)
	
	print("teamType:" .. teamType)
	for _ ,v in ipairs(MobaTeamData.GetDataByTeamType(teamType).memHero) do
		print("uid:" .. v.uid)
	end
	
    Global.Request(GetInterface("Category_pb"), GetInterface("SetArmyTeamRequestTypeID"), req, GetInterface("SetArmyTeamResponse"), function(msg)
...
local function LoadSelectList()
    ...
    for i, v in ipairs(mList) do
        if MobaTeamData.IsHeroSelectedByUid(teamType, v.msg.uid) then
    ...
    _ui.powerLabel.text = math.floor(MobaTeamData.GetTeamPower(teamType))
...
local LoadMyList
function LoadUI()
    MobaTeamData.NormalizeData()
    LoadMyList()
    LoadSelectList()
end

function LoadMyList()
    local heroListData = MobaHeroListData.GetSortedGenerals("3:100") -- HeroListData.GetData()
    local heroIndex = 1
    for _, v in ipairs(heroListData) do
        ...
        if not heroData.expCard then
			...
			if MobaHeroListData.IsOutForExpediation(v.uid) then -- bit.band(heroState, HeroListData.HeroStateSetout) ~= 0 then
				hero.stateBg.gameObject:SetActive(true)
                hero.stateList.setout.gameObject:SetActive(true)
                MobaTeamData.UnselectHero(teamType, v.uid)
			else
				hero.stateBg.gameObject:SetActive(false)
				hero.mask.gameObject:SetActive(MobaTeamData.IsHeroSelectedByUid(teamType, v.uid))
            end
            ...
            SetClickCallback(hero.btn.gameObject, function(go)
                ...
                local full = MobaTeamData.GetSelectedHeroCount(teamType) >= ChapterListData.GetHeroSlot()
                if MobaTeamData.IsHeroSelectedByUid(teamType, heroUid) then
                    MobaTeamData.UnselectHero(teamType, heroUid)
                    LoadUI()
                else
                    if full then
                        ...
                        if MobaTeamData.IsHeroSelectedByBaseId(teamType, heroBaseId) then
                            local text = TextMgr:GetText(Text.team_error_hero_repeat)
                            AudioMgr:PlayUISfx("SFX_ui02", 1, false)
                            FloatText.Show(text, Color.white)
                        elseif MobaHeroListData.IsOutForExpediation(v.uid) then -- bit.band(heroState, HeroListData.HeroStateSetout) ~= 0 then	
							local text = TextMgr:GetText(Text.BattleMove_atk)
                            AudioMgr:PlayUISfx("SFX_ui02", 1, false)
                            FloatText.Show(text, Color.white)
						else
                            MobaTeamData.SelectHero(teamType, heroUid)
                            LoadUI()
                        end
                    end
                end
...
function Awake()
        ...
        SetClickCallback(hero.btn.gameObject, function(go)
            ...
            for _, v in ipairs(myList) do
                if v.msg.uid == hero.msg.uid then
                    MobaTeamData.UnselectHero(teamType, v.msg.uid)
                end
            end
            LoadUI()
        end)
	...
	MobaSetoutData.AddListener(ReloadSelectList)
...
function Show(type)
	...
	--MobaTeamData.showHero(teamType)
end

function Close()
	_ui = nil
	MobaSetoutData.RemoveListener(ReloadSelectList)
