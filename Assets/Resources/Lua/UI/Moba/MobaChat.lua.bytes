module("MobaChat", package.seeall)
...
local function UpdateTabRedPoint()
	local new = MobaChatData.GetUnreadPrivateCount()
	...
	local guildNew = MobaChatData.GetUnreadGuildCount(0)
	bg_tab.guildRed.gameObject:SetActive(guildNew > 0 and curChanel ~= ChatMsg_pb.chanel_MobaTeam )
	bg_tab.guildRed:Find("num"):GetComponent("UILabel").text = guildNew
	if curChanel == ChatMsg_pb.chanel_MobaTeam then
		MobaChatData.SetUnreadGuildCount(0)
	end
end
...
local function SetContentAutoTranslate(chatinfo,chatContent, item)
	...
	if chatinfo.infotext ~= chatinfo.transtext and MobaChatData.GetTranslateEnable() then
...
local function SetChatInfoNormalContent(chatinfo, chatContent , item)
	if MobaChatData.AutoTranslate() and MobaChatData.GetTranslateEnable() then
...
function NotifyChanelChat()
	if GUIMgr:FindMenu("MobaChat") == nil then
		return
	end
	
	local chanelNewChat = MobaChatData.GetChanelNewChat(curChanel , 10 , recvList)
	...
	if chanelNewChat ~= nil and #chanelNewChat > 0 then
		...
		local setCorount = coroutine.start(function()
			for _, v in pairs(chanelNewChat) do 
				if MobaChatData.IsInBlackList(v.sender.charid) == false then
...
local function DeleteRecord(priName , priCharId)
	MessageBox.Show(TextMgr:GetText("Chat_priui1") , 
		function() 
			MobaChatData.DeleteChatData(priName , priCharId)
...
local function UpdatePrivateNGroupListData()
	...
	Global.SetMobaChatEnterChanel(ChatMsg_pb.chanel_MobaPrivate)
	...
	local contentItems = {}
	local sortTable = MobaChatData.GetPrivateNew()
	for i=1 , #(sortTable) , 1 do
		local k = sortTable[i].name
		local v = MobaChatData.GetPrivateChat(k)
		noitem = false
		--print("msg:" , k ,#(v))
		...
		local new = MobaChatData.GetPlayerUnreadPrivateContent(priName , priCharId)
	...
	for i=1 , #sortedCfg , 1 do
		...
		if contentItems[v.name] == nil then
			...
			local new = MobaChatData.GetPlayerUnreadPrivateContent(k , v.charid)
...
--	print(curChanel , updateChanel)
	if updateChanel ~= ChatMsg_pb.chanel_MobaWorld and updateChanel ~= ChatMsg_pb.chanel_MobaTeam and 
	...
	Global.SetMobaChatEnterChanel(optChanel)--记录聊天的页签
	...
	MobaChatData.InitRecordIndex(curChanel, recvList.name , recvList.charid , recvList.groupid)
	...
	if updateChanel == ChatMsg_pb.chanel_MobaPrivate then
		...
		chanelChat = MobaChatData.GetPrivateRecordData(recvList.name , recvList.charid, 10 , false)
		chatContentList.privatePanel.gameObject:SetActive(true)
		MobaChatData.SavePlayerChat(recvList.name , recvList.charid , true)
	elseif updateChanel == ChatMsg_pb.chanel_discussiongroup then
		chatContentList.privatePanel.gameObject:SetActive(true)
		chanelChat = MobaChatData.GetChanelRecordChat(curChanel , 10 , nil , nil , recvList.groupid)
		local gd = GroupChatData.GetGroupData(recvList.groupid)
		local curChatid = (#chanelChat > 0) and chanelChat[#chanelChat].id or 0
		GroupChatData.SetGroupCurrentChatId(gd.id , curChatid)
	else
		chanelChat = MobaChatData.GetChanelRecordChat(curChanel , 10 )
	end
	
	UpdateTabRedPoint()
	
	if updateChanel == ChatMsg_pb.chanel_MobaTeam then
		MobaChatData.SetUnreadGuildCount(0)
	end
...
function UpdateRecordChatInfo()
	...
	local chanelChat = nil
	if curChanel == ChatMsg_pb.chanel_MobaPrivate then
		chanelChat = MobaChatData.GetPrivateRecordData(recvList.name , recvList.charid, 10 , true)
	elseif curChanel == ChatMsg_pb.chanel_discussiongroup then
		chanelChat = MobaChatData.GetChanelRecordChat(curChanel , 10 , nil , nil , recvList.groupid)
	else
		chanelChat = MobaChatData.GetChanelRecordChat(curChanel , 10)
...
local function CheckLeaveUnion()
    if not UnionInfoData.HasUnion() then
        if not bg_tab[ChatMsg_pb.chanel_MobaWorld].transform:GetComponent("UIToggle").value then
            UpdateChatContent(ChatMsg_pb.chanel_MobaWorld)
            bg_tab[ChatMsg_pb.chanel_MobaTeam].transform:GetComponent("UIToggle").value = false
        end
    end
end
...
function Awake()
	...
	_ui = {}
	bg_tab = {}
	bg_tab[ChatMsg_pb.chanel_MobaWorld] = transform:Find("Container/bg_frane/bg_tab/btn_tabtype_5"):GetComponent("UIButton")
	...
	SetClickCallback(bg_tab[ChatMsg_pb.chanel_MobaWorld].gameObject, function(go)
		--curChanel = ChatMsg_pb.chanel_MobaWorld
		--UpdateChatContent()
		UpdateChatContent(ChatMsg_pb.chanel_MobaWorld)
	end)
	
	bg_tab[ChatMsg_pb.chanel_MobaTeam] = transform:Find("Container/bg_frane/bg_tab/btn_tabtype_4"):GetComponent("UIButton")
	...
	SetClickCallback(bg_tab[ChatMsg_pb.chanel_MobaTeam].gameObject, function(go)
		--curChanel = ChatMsg_pb.chanel_MobaTeam				
		--[[if not UnionInfoData.HasUnion() then
		...
		UpdateChatContent(ChatMsg_pb.chanel_MobaTeam)
	end)
	...
	MobaChatData.AddListener(NotifyChanelChat)
	--MobaChatData.AddListener(UpdateTabInfo)
...
print("chat start")
	...
	--[[if UnionInfoData.HasUnion() then
		bg_tab[ChatMsg_pb.chanel_MobaTeam].transform:GetComponent("UIToggle").enabled = true
	else
		bg_tab[ChatMsg_pb.chanel_MobaTeam].transform:GetComponent("UIToggle").enabled = false
	end]]
	bg_tab[ChatMsg_pb.chanel_MobaTeam].transform:GetComponent("UIToggle").enabled = true
	
	UpdateTabRedPoint()
	MobaMain.SetChatPreviewRedPoint(false)
	TranslateChatContent()
	
	local chatTab = Global.MobaGetChatEnterChanel()
	if privateChat ~= nil then
...
function Close()
	...
	MobaChatData.RemoveListener(NotifyChanelChat)
