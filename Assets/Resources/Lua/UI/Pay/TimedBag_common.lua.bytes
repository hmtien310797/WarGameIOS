module("TimedBag_common", package.seeall)
...
function Redraw(notwait)
    local config = TableMgr:GetGiftpackPopupConfig(iapGoodInfo.id)
    ...
    ui.currency.gameObject:SetActive(iapGoodInfo.priceType ~= 0)
    ...
    for i = 1, numItems --[[ math.max(math.min(numItems, MAX_ITEM_SHOW), #uiItems) ]] do
        if i > numItems then
...
function Awake()
    ...
    ui.currency = {}
    ui.currency.transform = transform:Find("container/btn_purchase/currency")
    ui.currency.gameObject = ui.currency.transform.gameObject
    ui.currency.icon = ui.currency.transform:GetComponent("UISprite")
    ...
    ui.itemList = UIUtil.LoadList(transform:Find("container/items/Scroll View"))
    -- ui.itemList.transform = transform:Find("container/items/grid")
	...
	ui.discount.transform.parent.localPosition = Vector3(72, 11, 0)
	ui.discount.transform.parent.localScale = Vector3(0.8, 0.6, 1)
    ui.discount.transform.parent.localEulerAngles = Vector3(0, 0, -16)
    ui.discount.text = System.String.Format(TextMgr:GetText("ui_discount"), iapGoodInfo.discount)
...
GiftPackData.RegisterPopupWindow(2, _M)
