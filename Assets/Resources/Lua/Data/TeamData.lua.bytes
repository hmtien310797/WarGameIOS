module("TeamData", package.seeall)
local teamData
...
function RequestData()
    ...
    Global.Request(Category_pb.Hero, HeroMsg_pb.HeroTypeId.MsgUserArmyUnitsRequest, req, HeroMsg_pb.MsgUserArmyUnitsResponse, function(msg)
        ...
        GeneralData.Initialize(msg)
        ArmyListData.SetData(msg.arms)
        ActiveHeroData.SetData(msg.actheros)
        ArmySetoutData.SetData(msg.setoutNum)

        HeroListNew.Initialize()
    end, true)
end
...
function IsHeroSelectedByBaseId(teamType, baseId)
    for _, v in ipairs(teamData.team) do
        if v.type == teamType or teamType == Common_pb.BattleTeamType_None  then
            for __, vv in ipairs(v.memHero) do
                local heroMsg = GeneralData.GetGeneralByUID(vv.uid) -- HeroListData.GetHeroDataByUid(vv.uid)
    ...
    for _, v in ipairs(teamData.team) do
        if v.type == teamType then
            for __, vv in ipairs(v.memHero) do
                ...
                power = power + GeneralData.GetPower(heroMsg) -- HeroListData.GetPower(heroMsg, heroData)
            end
        end
    end
    return power
...
function NormalizeData()
    for _, v in ipairs(teamData.team) do
        for i = #v.memHero, 1, -1 do
            local heroMsg = GeneralData.GetGeneralByUID(v.memHero[i].uid) -- HeroListData.GetHeroDataByUid(v.memHero[i].uid)
            if heroMsg == nil then
                v.memHero:remove(i)
            else
                local heroData = TableMgr:GetHeroData(heroMsg.baseid) 
...
function OnPushSetData(msg)
	-- HeroListData.SetData(msg.heros)
    GeneralData.SetData(msg)
    ArmyListData.SetData(msg.arms)
	ArmyListData.SetInjuredArmyData(msg)
	ArmySetoutData.UpdateData(msg.setoutNum)
	NotifyListener()
end
