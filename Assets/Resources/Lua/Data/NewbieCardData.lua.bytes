module("NewbieCardData", package.seeall)

local NEWBIE_TIME = 1209600 --新兵成长周卡可购买时间
local ONEDAY = 3600 * 24
local targettime = 0

function GetNewbieBuyTime()
    if targettime == 0 then
        targettime = MainData.GetCreationTime()
        targettime = (math.floor(targettime / ONEDAY) - 1) * ONEDAY + Global.GetFiveOclockCooldown(true)
        targettime = targettime + NEWBIE_TIME
    end
    return targettime - Serclimax.GameTime.GetSecTime() > 0 and targettime or 0
end
...
function SetData(userActivityInfoResponse)
    ...
    for _, userActivityInfo in ipairs(userActivityInfoResponse.activity) do
        ...
        if activityID == 3010 then
            RequestData(ShopMsg_pb.IAPGoodType_DevelopCard)

            shouldNotBeRemoved[ShopMsg_pb.IAPGoodType_DevelopCard] = true
        end
    end
...
function CanShow()
    return true--(GetNewbieBuyTime() > 0 and not availableCards[ShopMsg_pb.IAPGoodType_DevelopCard].goodInfo.hasBuy) or HasBought(ShopMsg_pb.IAPGoodType_DevelopCard)
end
-------------------------------------------------------------------
...
function ClaimAward(subType, callback)
    if subType == ShopMsg_pb.IAPGoodType_DevelopCard then
        local request = ShopMsg_pb.MsgIAPTakeDevelopCardRequest()

        Global.Request(Category_pb.Shop, ShopMsg_pb.ShopTypeId.MsgIAPTakeDevelopCardRequest, request, ShopMsg_pb.MsgIAPTakeDevelopCardResponse, function(response)
