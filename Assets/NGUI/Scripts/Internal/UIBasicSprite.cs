...
public abstract class UIBasicSprite : UIWidget
{
	...
	public enum FillDirection
	{
		...
		Radial180,
		Radial360,
		HorizontalUVAlignRight, //Chris Add for Hp bar
	}
	...
	override public int minWidth
	{
		get
		{
			if (type == Type.Sliced || type == Type.Advanced || type == Type.InverseSliced)
			{
...
#region Fill Functions
	...
	protected void Fill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Rect outer, Rect inner)
	{
		...
		switch (type)
		{
			...
			case Type.InverseSliced:
			InverseSlicedFill(verts, uvs, cols);
			break;
		}
	}
	...
	/// <summary>
	/// Sliced sprite fill function is more complicated as it generates 9 quads instead of 1. Keep center no change, fill borders.
	/// </summary>
	
	void InverseSlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		// get center size
		//Vector2 centerSize = new Vector2(tileSize.x - br.x - br.z, tileSize.y - br.y - br.w);

		// get new borders
		br.x = br.z = (v.z - v.x - tileSize.x) * 0.5f;
		br.y = br.w = (v.w - v.y - tileSize.y) * 0.5f;
	...
	void FilledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mFillDirection == FillDirection.Horizontal || mFillDirection == FillDirection.Vertical || mFillDirection == FillDirection.HorizontalUVAlignRight)
		{
			...
			//Chris Add for Hp bar
			else if (mFillDirection == FillDirection.HorizontalUVAlignRight)
			{
				float fill = (u.z - u.x) * mFillAmount;
