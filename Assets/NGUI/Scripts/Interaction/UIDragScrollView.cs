...
public class UIDragScrollView : MonoBehaviour
{
    ...
    public ScrollItemEx scrollItemEx;
	...
	void OnPress (bool pressed)
	{
        ...
        if (scrollItemEx && NGUITools.GetActive(this))
            scrollItemEx.Press(pressed);
    }
	...
	{
        //Debug.Log(delta.x + " drag  " + delta.y);
        bool isScrollItem = false;

        if (scrollItemEx && NGUITools.GetActive(this))
        {
            scrollItemEx.Drag(delta.x);
            isScrollItem = scrollItemEx.IsScroll;
        }

        if (scrollView && NGUITools.GetActive(this)&& !isScrollItem)
			scrollView.Drag();
    }
