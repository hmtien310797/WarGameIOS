...
public class UIScrollView : MonoBehaviour
{
    ...
    public delegate void ScrollItemExReset();
	/// <summary>
    ...
    public OnDragNotification onDragMove;
    ...
    public List<ScrollItemExReset> onItemReset = new List<ScrollItemExReset>();
    //public ScrollItemExReset onItemReset;
	...
	{
		mStarted = true;
        if (onItemReset != null) onItemReset.Clear();
    ...
    //define, restrict the scroll view's contents to be within the target transform
    public bool RestrictWithinBounds(bool instant, Transform target)
    {
        if (target == null)
            return false;

        Bounds boundss = NGUIMath.CalculateRelativeWidgetBounds(transform, target);

        Vector3 constraint = mPanel.CalculateConstrainOffset(boundss.min, boundss.max);

        if (scrollByIndex)
            constraint = -target.localPosition - mTrans.localPosition;

        if (constraint.sqrMagnitude > 0f)
        {
            SpringPanel.Stop(mPanel.gameObject);
    ...
    public bool RestrictWithinBoundsTop(bool instant) { return RestrictWithinBoundsTop(instant, true, true); }

    //reset to top
    public bool RestrictWithinBoundsTop(bool instant, bool horizontal, bool vertical)
    {
        //reset to start
        ...
        min.x = bounds.min.x - mPanel.clipSoftness.x;
        max.x = min.x + mPanel.baseClipRegion.z;

        min.y = bounds.min.y - mPanel.clipSoftness.y;
        min.y = min.y + mPanel.baseClipRegion.w;

        Vector3 constraint = mPanel.CalculateConstrainOffset(min, max);
    ...
    public bool RestrictWithinBoundsBottom(bool instant) { return RestrictWithinBoundsBottom(instant, true, true); }

    //reset to bottom
    public bool RestrictWithinBoundsBottom(bool instant, bool horizontal, bool vertical)
    {
        Vector2 max = Vector2.zero;
        Vector2 min = Vector2.zero;

        min.x = bounds.max.x + mPanel.clipSoftness.x;
        max.x = min.x - mPanel.baseClipRegion.z;

        min.y = bounds.min.y - mPanel.clipSoftness.y;
        max.y = min.y + mPanel.baseClipRegion.w;
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
                if (scrollByIndex)
                {
                    ...
                    while (mMomentum.magnitude > 0.0001f || mScroll != 0f)
                    {
                        if (movement == Movement.Horizontal)
                        {
                        ...
                        // Move the scroll view
                        offset += NGUIMath.SpringDampen(ref mMomentum, 9f, delta);
                    }

                    Vector3 move = mTrans.InverseTransformPoint(offset) - mTrans.InverseTransformPoint(Vector3.zero);
                    ...
                    {
                        x = -(int)((mTrans.localPosition.x + move.x) / (eachSize.x / 2f));
                    }

                    if (x < 0)
                        x = 0;
                    else if (x >= mTrans.childCount)
                        x = mTrans.childCount - 1;

                    if (hideInactive)
                    {
                        if(mTrans.GetChild(nowIndex = x).gameObject.activeSelf)
                            RestrictWithinBounds(false, mTrans.GetChild(nowIndex = x));
                    }
                    else
                        RestrictWithinBounds(false, mTrans.GetChild(nowIndex = x));
                }
			...
			else
			{
                ...
                if (scrollByIndex)
                {
                    ...
                    x = Mathf.Abs(-(int)(mTrans.localPosition.x / eachSize.x));

                    float lx = -eachSize.x * x + eachSize.x * 0.5f;
                    float rx = -eachSize.x * x - eachSize.x * 0.5f;

                    if (mTrans.localPosition.x > lx)
                        x--;
                    else if (mTrans.localPosition.x < rx)
                        x++;
    ...
    public void ResetScrollItemEx()
    {
        //if (onItemReset != null)
        //    onItemReset();
        foreach(var itemReset in onItemReset)
        {
           itemReset();
        }
    }
    public void ClearScrollItemList()
    {
        if (onItemReset != null)
        {
            for (int i = 0; i < onItemReset.Count;i++)
            {
                onItemReset[i] = null;
            }
            onItemReset.Clear();
    ...
    public bool                 scrollByIndex           = false;
    public int                  nowIndex                = 0;
    public Vector2              eachSize                = Vector2.zero;
    public bool                 scrollOne               = true;
    public float                scrollOffset            = 10f;
