...
public class UILabel : UIWidget
{
    ...
    public float textWidth()
    {
        ...
        mTrueTypeFont.RequestCharactersInTexture(mText, fontsize, FontStyle.Normal);
        CharacterInfo characterInfo;
        float width = 0f;
        for (int i = 0; i < mText.Length; i++)
        {

            mTrueTypeFont.GetCharacterInfo(mText[i], out characterInfo, fontsize);
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (effectStyle != Effect.None)
		{
			...
			if ((effectStyle == Effect.Outline) || (effectStyle == Effect.Outline8) || (effectStyle == Effect.OutlineAndShadow))
			{
				offset = end;
				end = verts.size;
                ...
                else if (effectStyle == Effect.OutlineAndShadow)
                {
                    offset = end;
                    end = verts.size;

                    ApplyShadow(verts, uvs, cols, offset, end, 0, -pos.y - 2);

                    offset = end;
                    end = verts.size;

                    ApplyShadow(verts, uvs, cols, offset, end, pos.x, -pos.y - 2);
