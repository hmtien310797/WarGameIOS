...
public class UIRoot : MonoBehaviour
{
	...
	public int activeHeight
	{
		get
		{
			...
			else
			{
                ...
                {
                    // for iPad, no scale for ipad2, ipad pro
                    if (aspect > 1f && aspect <= 1.49f)
                    {
                        if (screen.y >= maximumHeight)
                            return maximumHeight;
                        else if (screen.y > minimumHeight)
	...
	{
#if PROFILER
        Profiler.BeginSample("UIRoot_Update");
#endif
#if UNITY_EDITOR
