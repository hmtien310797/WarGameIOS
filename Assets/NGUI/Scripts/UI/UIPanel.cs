...
{
    //额外的附加渲染队列 范围 无论该panel是否拥有可绘制的dc 都将附加这个范围
    //只要用于在UI界面中为3D物件预留空间
	...
	{
#if PROFILER
        Profiler.BeginSample("UIPanel_LateUpdate");
#endif
#if UNITY_EDITOR
...
#endif
		{
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				...
				if (p.renderQueue == RenderQueue.Automatic)
				{
					...
					p.UpdateDrawCalls();
					rq += p.drawCalls.Count+Mathf.Max(0, p.AddRQueueRange);//添加了额外rq范围
				}
				else if (p.renderQueue == RenderQueue.StartAt)
				{
					p.UpdateDrawCalls();
					if (p.drawCalls.Count != 0)
						rq = Mathf.Max(rq, p.startingRenderQueue + p.drawCalls.Count+Mathf.Max(0, p.AddRQueueRange));//添加了额外rq范围
				}
				else // Explicit
				{
					p.UpdateDrawCalls();
					if (p.drawCalls.Count != 0)
						rq = Mathf.Max(rq, p.startingRenderQueue + 1+Mathf.Max(0, p.AddRQueueRange));//添加了额外rq范围
				}
			}
		}
#if PROFILER
