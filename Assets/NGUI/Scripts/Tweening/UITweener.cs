...
public abstract class UITweener : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (ignoreTimeScale && delta > 0.1f) delta = 0.1f;
    ...
    {
        if (onFinished != null) onFinished.Clear();
	...
	public void Play (bool reset = false) { Play(true, reset); }

	/// <summary>
	/// Play the tween forward.
	/// </summary>

	public void PlayForward (bool reset = false) { Play(true, reset); }

	/// <summary>
	/// Play the tween in reverse.
	/// </summary>
	
	public void PlayReverse (bool reset = false) { Play(false, reset); }

	/// <summary>
	/// Manually activate the tweening process, reversing it if necessary.
	/// </summary>

	public void Play (bool forward, bool reset)
	{
	...
	// Chris add: Convinience methods to play all UITweeners
	public static void PlayAllTweener(GameObject obj, bool forward, bool reset = false, bool includeChildren = false)
	{
		if (obj == null)
			return;

		UITweener[] tweeners = includeChildren? obj.GetComponentsInChildren<UITweener>(): obj.GetComponents<UITweener>();
		for (int i = 0, imax = tweeners.Length; i < imax; ++i)
		{
			tweeners[i].Play(forward, reset);
		}
	}

	public static void ResetAllToBegining(GameObject obj, bool includeChildren = false)
	{
		if (obj == null)
			return;
    ...
    public static void PlayAllTweenerDestory(GameObject obj, bool forward, bool reset = false, bool includeChildren = false, bool destroyImmediate = false)
    {
        if (obj == null)
            return;
        ...
        //destrory
        List<UITweener> lst = new List<UITweener>(tweeners);
        lst.Sort
        (
            (UITweener _pre, UITweener _next) =>
            {
                return _next.duration.CompareTo(_pre.duration);
            }
        );
        UITweener tweener = lst[0];
        tweener.onFinished.Add
        (
            new EventDelegate
            (
                () => 
                {
                    Debug.Log("EventDelegate");
                    if(destroyImmediate)
