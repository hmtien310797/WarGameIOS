...
public class Reporter : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        if (!show)
        {
            if (numOfCollapsedLogsError > 0)
            {

                GUI.color = Color.red;
                GUI.Label(new Rect(0, 0, 200, 50), string.Format("error count:{0}", numOfCollapsedLogsError));
            }
            ...
            if (showNetworkLog)
            {
                ...
                GUI.Label(new Rect(100, 0, 1000, 50), string.Format(
                    "last send:{0:N0}B\t last recv:{1:N0}B\t total send::{2:N0}B\t total recv:{3:N0}B\t total resp count:{4}\t last resp:{5:N0}ms\t avg resp:{6:N0}ms\t",
                    network.lastSendBytes,
                    network.lastReceiveBytes,
                    network.totalSendBytes,
                    network.totalReceiveBytes,
                    network.totalResponseCount,
                    network.lastResponseTime * 1000,
                    network.averageResponseTime * 1000
                    ), evenLogStyle);
