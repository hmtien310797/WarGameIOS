...
Shader "TSM2/Base VL Phong" 
{
    Properties 
    {
		...
		_PhongFactor("phong factor",Range(0,1)) = 0.8
        ...
        Pass 
        {
            ...
            CGPROGRAM
                ...
                v2f vert (appdata_base0 v)
                {
					...
					o.normal = v.normal;
					//o.lightColor = pow(o.lightColor, 2);
                ...
                fixed4 frag (v2f i) : COLOR
                {
					...
					fixed r = reflect(-i.lightDir,i.normal);
					fixed spec = max(_PhongFactor,dot(r,i.viewDir));
					spec = pow(spec,4)*_SpecFactor;
					fixed diff = max(_PhongFactor,dot(i.normal,i.lightDir));
					...
					col *= fixed4(i.lightColor, 1)*(diff + spec);
